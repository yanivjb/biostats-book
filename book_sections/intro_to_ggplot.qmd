...
# 2. Introduction to ggplot {#intro_to_ggplot}
...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/1HVyPwIGFuYn5O-i_937SwAQ3BKSKsM_R/view?usp=sharing) &nbsp;·&nbsp; 🎧 [Listen](../audio/intro_to_r/intro_to_ggplot.mp3)
:::
...
**Detect data quality issues early**  It is common to split up data collection to a team and maybe someone enters data in centimeters, and someone else in inches (etc). Or maybe for a data point or two a decimal point was lost,  a value is way bigger than it should be. Every time  you collect and enter data you should make some plots to help identify any such data issues.
...
Before starting your R coding, consider the plots you want to make and why. It is all too easy to get stuck in the  R vortex—making plots simply because they’re easy to create, visually appealing, fun, or challenging, and before you know it you've wasted an hour doing something that did not move your analysis or understanding forward. So before making a (set of) plot(s) always consider   
...
- **The thing you want to know**, and how it relates to the motivating biology. This includes
   ...
   - Distinguishing between key explanatory variables from covariates that you want to account for, but are not of primary interest.  
...
- We also may want to evaluate the extent to which the correlation between traits were broken up  as we made the RILs. If trait correlations persisted in the RILs, it means that we cannot fully dissect the contribution of each trait to the outcome we care about, and that the genetic and/or physiological linkage between traits may have prevented evolution from acting independently on these traits.    
...
```{r}
#| fig-cap: "Watch [this video](https://www.youtube.com/watch?v=cqcQT44mJTM) about getting started with ggplot2 (It is 7 min and 17 sec long), from STAT 545."
#| echo: false
