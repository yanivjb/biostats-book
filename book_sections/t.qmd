# 15. The t distribution {.unnumbered #t}
...
:::aside
The subscript in $\mu_0$ indicates that this is the hypothesized mean under the null hypothesis.
:::

This allowed us to summarize the distance between an estimated mean ($\bar{x}$) and its null value, ($\mu_0$) in standardized units (standard errors of the mean), regardless of the scale of the original data.  This is super useful, because we can conduct null hypothesis significance testing and similar analyses. 

*However, there’s a problem:* We usually have only a sample and not the whole population (that's why we're doing statistics after all!). So we have an **estimate** of the standard deviation ($s$) and not its true parameter value ($\sigma$). This means our standard error is also an estimate and therefore uncertain.  Unfortunately, the Z-distribution does not account for this additional uncertainty.
...
Because the tails are "fatter",  a larger proportion of the area lies beyond $\pm2$ (greater than 4.55%, compared to approximately 4.55% for the Z-distribution).   
...
```{r}
#| label: fig-tvz 
#| fig-cap: "Comparing the t (black) and standard normal (Z, red) distributions across different degrees of freedom."
...
The answers to these questions depend on which t-distribution we're talking about. There are many, each associated with a different number of degrees of freedom. 

Degrees of freedom reflect the number of values free to vary, which relates to the uncertainty in our estimate. So, the more degrees of freedom, the more certainty! As such, tails become less "fat," and the t-distribution looks more and more like the standard normal (Z) distribution as the degrees of freedom increase (@fig-tvz).
...
:::fyi
**You may see Z-tests in the literature.** With a sample, a Z test is not 100% correct. But, Z-tests are often "right-ish". As sample sizes get large, the t- and Z- distributions converge. This is good enough for me. My view is that you should worry about big problems that will mislead you, not modest approximations that are close to, but not exactly right.
...
* We then [calculate 95% confidence intervals for data modeled by the *t*-distribution](#t_CI), and show how the ["one sample" *t*-test](#one_sample_t_test) can be used to evaluate the null hypothesis that a sample comes from a population with mean $\mu_0$. After working through the math and logic of this "one-sample *t*-test," we see how [R can do most of this heavy lifting for us](#one_sample_t_test_in_R).
...
* As always, we conclude with a [chapter summary](#t_summary).
