## •  17. *F*-inding connec*t*ions  {.unnumbered  #finding_connections}
...
**Motivating Scenario:**
You earlier saw how to compare two groups with a two-sample t-test. Then we just basically did the same thing as an ANOVA. Now, you are curious about the relationship between these approaches and why they give the same answer.
...
This question is commonly asked by students with data in hand, and even by professors on an exam to evaluate students' understanding. The answer is: for a binary explanatory variable, it makes no difference.    

Reassuringly, whether we conduct an ANOVA or a t-test on a continuous response and a binary explanatory variable, we get the exact same p-value. This is reassuring, as we would like our answers to be robust to such an arbitrary choice. It's up to you whether you prefer to present results in terms of the number of standard errors between group means, or as a ratio of mean squares. 

Here we connect these modeling approaches. 
...
- Finally, although R did not show us this, the Mean Square error of  0.0000239 equals the pooled variance: 
...
In short, the two-sample t-test and a two-group ANOVA are mathematically identical. I hope this makes us feel good about moving on to more complex ANOVAs, and about linear models more broadly.
...
Because variance is the square of a standard deviation, F is the square of the corresponding t when there are two groups.  
...
You may also remember that $r$ is the correlation between variables. If we assign a numeric value of zero to the explanatory variable for one category (e.g., white flowers), and one for the other category (e.g., pink flowers), we can calculate a correlation ($r$). Squaring this correlation reveals that $r^2$ equals $R^2$!   
