## •  17. F Calculations {.unnumbered  #f_calcs}
...
- Interpret the ratio of mean squares as an F statistic, connect F to the idea of taking multiple samples from the null, and use this to test the null hypothesis. 
...
Let's look at the association between petal color and admixture proportion of natural *parviflora* plants in the SM hybrid zone. The motivating idea is: if  white petals do not attract pollinators (as seen in the previous chapter), white plants will have had less admixture (because they would not receive *xantiana* pollen). 
...
#| column: page-right
#| label: fig-devs-a 
#| fig-cap: "Partitioning the contribution of petal color and error to admixture proportion of pink and white parviflora plants in the Squirrel Mountain hybrid zone. **(A)** Model deviation: Lines show the distance between each sample's predicted value (i.e., the group mean) and the grand mean (black line). **(B)** Error (residual) deviation: Lines show the distance between each observation and its predicted value (i.e. group means). **(C)** Total deviation: Lines show the distance between each observation and the grand mean."
...
Before calculating these sums of squares and mean squares, it's worth visualizing the data and where these deviations come from. @fig-devs-a shows:     
...
- B. **"Error (residual) deviation":** The difference between each predicted value ($\hat{Y}_i$ i.e. the group mean) and the observed value, $Y_i$.      Squaring and summing each such deviation provides the $\text{SS}_\text{Error} = \sum{(\hat{Y}_i-{Y}_i)^2}$.     <br>  <br>  

- C. **"Total  deviation":** The difference between each observed value, $Y_i$, and the grand mean. Note that this is the sum of the model and error (residual) deviation. Squaring and summing each such deviation  provides the $\text{SS}_\text{Total} = \sum{({Y}_i-\bar{Y})^2}$.    <br>  <br>  


## Calculating SS and MS from @fig-devs-a


In each panel of @fig-devs-a, we can square the length of each line and add them up to find each of the relevant sums of squares! But first we need to find the lengths of these lines. In R we can do this by:   

- Building a linear model (See [Chapter 13](#linear_models)),     
...
We can use the `lm(RESPONSE ~ EXPLANATORY, data = DATA)` function in R to build a model of admixture proportion as a function of petal color:
...
As a crash course in how to read this:  
...
:::aside
$\hat{Y}_\text{white}$ = $`r sm_lm$coefficients[["(Intercept)"]] |> round(digits = 4)`$ + $`r sm_lm$coefficients[["petal_colorwhite"]] |> round(digits = 4)`$  $= `r (sm_lm$coefficients[["(Intercept)"]] |> round(digits = 4)+sm_lm$coefficients[["petal_colorwhite"]]) |> round(digits = 4)`$.
:::
...
Now that we have our model, we can use `broom`'s [`augment()`](https://broom.tidymodels.org/reference/augment.lm.html) function to extract key information about each plant.  Specifically, we focus on    
...
Remember that testing the null hypothesis is only one goal of statistics -- we also want to know the extent to which our explanatory variable(s) modulate the value of the response variable.  $R^2$ quantifies the overall effect of predictors  on the response variable as:
...
Because the p-value is much smaller than our traditional $\alpha$ of 0.05. We resoundingly reject the null hypothesis. In fact, if we generated one sample from the F distribution every second, we would need to wait for more than 900 years until a random sample was this extreme. 
