## • 17. F (ANOVA) summary {.unnumbered #f_summary}
...
Links to: [Summary](#f_summary_chapter-summary). [Chatbot tutor](#f_summary_chatbot_tutor).  [Questions](#f_summary_practice-questions). [Glossary](#f_summary_glossary-of-terms). [R packages](#f_summaryR_packages-introduced). [R functions](#f_summary_key-r-functions). [More resources](#f_summary_additional-resources).
...
Please interact with this custom chatbot ([**link here**](https://chatgpt.com/g/g-68db186815108191a327731289b4d1e4-f-tutor)). I have made it to help you with this chapter. I suggest interacting with at least ten back-and-forths to ramp up  and then stopping when you feel like you got what you needed from it. 
...
## Practice Questions {#f_summary_practice-questions}
...
*In this chapter we used the ANOVA to test the null hypothesis that pink and white parviflora flowers have the same admixture proportion at Squirrel Mountain.*  
...
## 📊 Glossary of Terms {#f_summary_glossary-of-terms}
...
* **[`aov()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/aov.html)`**:
  Fits an analysis of variance model directly.


* **summary()**:   Use [`summary()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/summary.lm.html) on the resulting object to view the ANOVA table, including sums of squares, mean squares, F statistic, and p-value.
...
* **[`anova()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/anova.html)**:
...
```
  ...
  This approach is mathematically identical to [`aov()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/aov.html) for one-way ANOVA, but uses a linear model object as input.
...
* **`tidy()`** and **`glance()`** *(from the [`broom`](https://broom.tidymodels.org/) package)*:

  * [`tidy()`](https://generics.r-lib.org/reference/tidy.html) formats the ANOVA table into a tidy data frame.
  * [`glance()`](https://generics.r-lib.org/reference/glance.html) returns model-level summaries, including ($R^2$).
...
## Additional resources   {#f_summary_additional-resources}
...
[Khan Academy ANOVA](https://www.khanacademy.org/math/statistics-probability/analysis-of-variance-anova-library/analysis-of-variance-anova/v/anova-1-calculating-sst-total-sum-of-squares) 
