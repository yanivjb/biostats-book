...
- **Transformed values must maintain a monotonic relationship with the original values.** This means that the order of values should be consistent — larger values in the original data should remain larger after transformation (it's also ok if large become small and small become large, so long as you interpret this correctly, and everyone stays in line).  
