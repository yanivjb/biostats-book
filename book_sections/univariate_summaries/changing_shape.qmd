...
**Motivating Scenario:**    Your data are not symmetric, and you are considering changing its shape. Here I introduce some thoughts and guidance about common data transformations and when to use them. 
...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/13D0x9aa8yfiqg_-xIP0j0HZVuxSJaGJh/view?usp=sharing) &nbsp;·&nbsp; 🎧 [Listen](../../audio/intro_to_r/univariate_summaries/shanging_shape.mp3)
:::
...
*There is nothing inherently "natural" about the linear scale.* In fact, [some research (review by @asmuth2018)](https://onlinelibrary.wiley.com/doi/10.1111/cogs.12615) suggests that humans naturally think in log scale, and only begin thinking in linear scale with more formal schooling. That is, kids tend to think that the difference between seventy-five and eighty is smaller than the difference between five and ten.    

- In a sense, the kids are right. Ten is double five, while eighty is only $\frac{16}{15}$ of seventy-five.    
- Of course, in another sense, they’re wrong. The difference is five in both cases.   
...
By contrast, variables that exhibit diminishing returns or are constrained by hard limits often result in left-skewed distributions. We saw this in our genotyping efforts, where we aimed to genotype eight seeds per mom, and stopped once we got eight. But we sometimes had fewer than eight seeds as some moms only had e.g. six seeds.
...
When the underlying biological process results in nonlinear data with asymmetric distributions, transforming the data is often appropriate (even if you don't know the mechanism underlying this nonlinearity). But being statistically valid is one thing; being a good idea is another thing. 
...
- **Transformed values must have a one-to-one correspondence with the original values.** For example, don’t square values if some are negative and others positive, as you'll lose information about the sign of the data.  
- **Transformed values must maintain a monotonic relationship with the original values.** This means that the order of values should be consistent i.e. larger values in the original data should remain larger after transformation (it's also ok if large become small and small become large, so long as you interpret this correctly, and everyone stays in line).  
...
Transformation in R is remarkably straightforward. Simply use [`dplyr`](https://dplyr.tidyverse.org/)'s [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html) function to add a transformed variable. Below, I show a log base 10 transformation, which is one of the most common and useful approaches for right-skewed data.  @fig-log10 shows that this $log_{10}$ transformation changes the shape of the petal area distribution from right-skewed to roughly symmetric.
...
| Transform             | Formula        | What it does                                         | Data it's good for                        | Example datasets                               | R function (Combine with `mutate()`)                                                                                                 | Limitations                         |
|--------|-------------------------------|------------------------------------------------------|-------------------------------------------|------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------|--------------------------------------|
...
| Exponential           | $e^x$             | Increases spread, emphasizes large values            | Left-skewed data                          |                  | `exp(x)`                                                   | Makes values more extreme           |
| Cube              | $x^3$             | Further increases spread                             | Left-skewed data                          | Tree volume, growth rates                      | `x^3`                                                     | Strongly affects scale              |
