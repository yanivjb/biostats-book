...
Links to: [Summary](#summarizing_summaries_chapter-summary). [Chatbot tutor](#summarizing_summaries_summaryChatbot) [Questions](#summarizing_summaries_practice-questions). [Glossary](#summarizing_summaries_glossary-of-terms). [R functions](#summarizing_summaries_key-r-functions). [R packages](#summarizing_summaries_r-packages-introduced). [More resources](#summarizing_summaries_additional-resources).  


::: {style="font-size: 0.85em; color: #6b7280; margin-top: 0.8em; margin-bottom: 0em;"}

**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/1BUHpJHt1k4CtKhaBuuLhhX495PHaCmop/view?usp=drive_link) &nbsp;·&nbsp; 🎧 [Listen](../../audio/intro_to_r/univariate_summaries/summarizing_summaries.mp3)
...
### Chatbot tutor   {#summarizing_summaries_summaryChatbot}  

:::tutor
Please interact with this custom chatbot ([**link to ChatGPT bot here**](https://chatgpt.com/g/g-683138e4af688191a0d0fe7c9a3badc4-univariate-summaries-tutor), [**link to Gemini bot here**](https://gemini.google.com/gem/1duzWKm4d7AC_XNcd2xckfu3iaNEG6J47?usp=sharing)) I have made to help you with this chapter. I suggest interacting with at least ten back-and-forths to ramp up  and then stopping when you feel like you got what you needed from it. 
:::
...
penguins |>
  summarise(n_samples = n(),
     ...
     mean_mass_2 = mean(body_mass, na.rm = T))
...
**Q4)** What went wrong in calculating these means?  `r longmcq(c(answer = "n()  counts the number of entries, but we need the number of non-NA entries.",  "na.rm should be set to TRUE, not T", "The denominator for the mean is (n - 1), not n.", "Nothing -- they are both potentially correct depending on your goals."))`

---

**Use the head start provided in the webR environment below to answer:**  
...
**Calculate the coefficient of variation as $sd_x / \bar{x}$.** So... e.g.:  
...
library(dplyr)
penguins  |> 
  summarise(sd_mass   =  sd(body_mass, na.rm = T),
            var_mass  = var(body_mass, na.rm = T),
            mean_mass = mean(body_mass, na.rm = T),
...
adelie  |> 
  summarise(sd_mass   =  sd(body_mass, na.rm = T),
            var_mass  = var(body_mass, na.rm = T),
            mean_mass = mean(body_mass, na.rm = T),
            #THING_HERE = MATH HERE)
  )
```


::: aside
**NOTE:** Here we find separate results for each species by analyzing different, tibbles each [`filter()`](https://dplyr.tidyverse.org/reference/filter.html)ed by a different species. In the next chapter we will see how to [`group_by()`](https://dplyr.tidyverse.org/reference/group_by.html) a categorical variable to smoothly group category specific summaries.
:::
...
**Q6a)** The mean is roughly   `r longmcq(c("Six",  "One and three quarters", "Five hundred and seventy nine" , answer =  "We cannot estimate the mean from this plot."))`


**Q6b)** The median is roughly   `r longmcq(c("Six",  "One and three quarters", answer =  "Five hundred and seventy nine" , "We cannot estimate the median from this plot."))`

**Q6c)** The mode is roughly   `r longmcq(c("Six","One and three quarters",  "Five hundred and seventy nine" ,  answer =  "We cannot estimate the mode from this plot."))`
...
**Brooke planted RILs at four different locations,  and found tremendous variation in the proportion of hybrid seed across locations.** The first step in quantifying this variation is to calculate the sum of squares, so let's do it. Use the image below (@fig-ss) to calculate sum of squares for proportion hybrid seeds across plants planted at four different locations.
...
**Q7d)**  Accounting for differences in their means, how does the variability in the proportion of hybrid seed across locations compare to the variability in petal area among RILs? (refer to the section on [summarizing variability ](#summarizing_variability) for reference).  `r longmcq(c( "They are very similar", "Petal area among RILs is roughly thirty times as variable", answer = "The proportion of hybrid seed among sites is roughly two times as variable", "You cannot compare variability for different traits measured on such different scales" ))` 
...
<html><textarea rows="6" cols="50" placeholder="Your answer here..."></textarea></html>         
...
**Q9)** What is wrong with the code below? (Pick the most egregious issue).  
...
`r longmcq(c(answer = "I overwrote iris and lost the raw data", "I did not show the output", "I used summarise() rather than summarize()",  "I did not tell R to remove missing data when calculating the mean."))`
...
- **Mean Absolute Deviation (MAD):** The average absolute deviation from the mean or median. Robust and intuitive.  
...
- **[`mad()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/mad.html)** *([base R])*: Computes the **median absolute deviation** (not the mean) — a robust summary of variability.  
