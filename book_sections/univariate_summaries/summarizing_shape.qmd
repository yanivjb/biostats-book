...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/13D0x9aa8yfiqg_-xIP0j0HZVuxSJaGJh/view?usp=sharing) &nbsp;·&nbsp; 🎧 [Listen](../../audio/intro_to_r/univariate_summaries/summarizing_shape.mp3)
:::
...
#| fig-alt: "A photograph of two *Clarkia xantiana* flowers placed on a ruler for scale. The flower on the left is way bigger."
include_graphics("../../figs/intro_to_r/univariate_summaries/1969.png")
```
...
#| warning: false
#| column: page-right
#| fig-cap: "*Examples of three distribution shapes:* **(A)** The proportion of hybrid offspring is right-skewed, **(B)** The number of offspring genotyped is left-skewed, and **(C)** log-transformed petal area is approximately symmetric."
#| fig-alt: "Three histograms illustrating distribution shapes. A: prop_hybrid is strongly right-skewed, with most observations near zero and progressively fewer observations toward one. B: offspring_genotyped is strongly left-skewed, with nearly all observations at eight and a few lower values between three and seven. C: log10_petal_area_mm is approximately symmetric and bell-shaped, centered near 1.8."
library(patchwork)
...
:::fyi
**Data transformations and skew:**  @fig-shape showed that $\text{log}_{10}$ petal area was roughly symmetric. The careful reader might be suspicious and wonder why I transformed the data. The answer is that area is usually right skewed and log transforming often removes such skew. 

We discuss changing the shape of distributions by transformation in the [next section](#changing_shape). Later we will see that symmetric data are often easier for statistical models than skewed data so such transformations are common.
...
One of the first things to notice when visualizing a dataset is the **number of modes**. Unimodal data cluster around a single peak, while multimodal data contains numerous peaks. The number of modes can point to interesting biology (e.g.  distinct subgroups).    
...
- Of course, distributions can have more than two modes. **Trimodal distributions** have three peaks, and so on. However, be careful not to over-interpret small fluctuations, as small dips can create false peaks from random variation. It's worth experimenting with bin size to avoid overinterpreting such small blips.
