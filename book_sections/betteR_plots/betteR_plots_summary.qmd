## • X2. Better ggplots summary {.unnumbered #better_plots_summary}
...
`r hide("Hint")`
In the `geom_*_` Set  the `height` argument to zero and provide a small `width` (e.g., `0.2`)
`r unhide()`
...
**Q7)** Now you want the species names (e.g. Gentoo, Chinstrap, etc) but not flipper lengths (e.g. 170, 210, etc.) to be italicized. What would you add to the theme to do this? (*NOTE* we did this a different way in the chapter)  `r longmcq(c("element_text(size = 16, face = 'italic')", answer = "axis.text.x = element_text(size = 16, face = 'italis'), axis.text.y = element_text(size = 16)"))`
...
- **[`stat_summary()`](https://ggplot2.tidyverse.org/reference/stat_summary.html)**: Calculates summary statistics (like means or confidence intervals) on the fly and adds them to the plot as a new layer (e.g., as bars or errorbars). This function is in the [`ggplot2`](https://ggplot2.tidyverse.org/) package, but requires the [`Hmisc`](https://hbiostat.org/r/hmisc/) package.      
...
- **[`plot_annotation()`](https://patchwork.data-imaginist.com/reference/plot_annotation.html)** In the [`patchwork`](https://patchwork.data-imaginist.com/) package: Adds overall titles and panel tags (e.g., A, B, C) to a combined figure.  

- **[`plot_layout()`](https://patchwork.data-imaginist.com/reference/plot_layout.html)** *In the [`patchwork`](https://patchwork.data-imaginist.com/) package:* Controls the layout of a combined figure, such as collecting all legends into a single area.
...
#### ✨ **Flair (Images, Animations & Themes)**   
...
**[`ggblend`](https://mjskay.github.io/ggblend/)**: A ggplot extension for blending and composing overlapping layers. This is useful when there are a lot of overlapping points in different categories. 
...
- **[The `ggplot2` Extensions Gallery](https://exts.ggplot2.tidyverse.org/gallery/)**: The official showcase of packages that extend the power of `ggplot2` with new geoms, themes, and capabilities. A good place to discover new visualization tools in the ggplot family.   
