## • X2. Making cleaR plots {.unnumbered #cleaR_plots}
...
2.  **Ensure labels are clear and informative** by:
    ...
    * Using [`labs()`](https://ggplot2.tidyverse.org/reference/labs.html?q=labs#null) to provide descriptive axis and legend titles.
    * Using [`scale_*_manual()`](https://ggplot2.tidyverse.org/reference/scale_manual.html) to rename shorthand categories in a legend.  
 
3.  **Represent data points honestly and guide the reader's eye** with:
    * Controlled [`geom_jitter()`](https://ggplot2.tidyverse.org/reference/geom_jitter.html) to avoid overplotting without distorting the data's meaning.
    * [`stat_summary()`](https://ggplot2.tidyverse.org/reference/stat_summary.html) to add visual summaries like means and error bars.     
...
#| fig-cap: "Our starting plot - now with flipped axes and better labels. The legend labels are unclear, data points are all over the place, but now we can read the categories and know what X and Y mean, so that's something."
...
- Add the [`stat_summary()`](https://ggplot2.tidyverse.org/reference/stat_summary.html) layer before [`geom_jitter()`](https://ggplot2.tidyverse.org/reference/geom_jitter.html) to ensure the raw data points are plotted on top of the bars. 
...
- *What the bars should show* I usually choose 95% Confidence intervals (more on that in a later chapter) with`fun.data = "mean_cl_normal"`.   
   - **NOTE:** Standard errors, standard deviations, 95% confidence intervals and the like are all different things. But all can be shown with bars. So you must communicate what the bars represent. I usually do this in the figure legend.        
...
**Challenge:** Change the code so that `site_ssp` is ordered first by subspecies and then by longitude.
