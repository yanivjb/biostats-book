## • X2. Plots for the medium {.unnumbered #plots_foR_medium}
...
2.  **Prepare publication-quality figures for scientific papers** by:
    ...
    * Formatting labels with mathematical symbols or italics using [`expression()`](https://stat.ethz.ch/R-manual/R-devel/library/grDevices/html/plotmath.html) and/or [`ggtext`](https://wilkelab.org/ggtext/articles/introduction.html).
...
R is great because it is reproducible and dependable.  What's more,  once you can do something in R, it's pretty straightforward to apply that skill to a new dataset. This comes in handy  when you'll be doing the same task a bunch. But figuring out how to do a very specific, one-off tweak in R can be a huge time sink, and sometimes it makes more sense to make final touches in powerpoint, illustrator, or photoshop than to figure it out in R. Before you spend hours on a minor customization, ask yourself a few questions. Here is the guidance I use when making this decision:
...
**Plots in scientific talks require large text!** We achieve this largely through the [`theme()`](https://ggplot2.tidyverse.org/reference/theme.html) function where we use `element_text()` to customize a specific feature of our plot. 
...
- Next, copy and paste the code from the margin on the right into the session to see how [`theme()`](https://ggplot2.tidyverse.org/reference/theme.html) can change the text size.     

- *Challenge:* You will notice the y-axis tick mark labels are still small. Add a line of code inside [`theme()`](https://ggplot2.tidyverse.org/reference/theme.html) to make the axis.text.y larger as well.      
...
- Using the [`expression()`](https://stat.ethz.ch/R-manual/R-devel/library/grDevices/html/plotmath.html)  function, which is more "classic R" but a bit of a pain. 
