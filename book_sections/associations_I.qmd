...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/19eLAMqdmEFaXqCrxhfmbsJiYQCoBgaYA/view?usp=sharing) &nbsp;·&nbsp; 🎧 [Listen](../audio/intro_to_r/associationsI.mp3)
:::
...
- It looks like pink flowers have a way better chance of receiving at least one pollinator than white flowers do.    
...
Of course, our ultimate ambition is to do more than just characterize the association between variables. Rather, we ultimately aim to know what such associations imply. We want to:      

- Predict one variable from another.   
- Anticipate what some intervention will do to a biological system. 

In future chapters we will see when and how we can achieve these higher goals. But for now, know that a correlation is just a description. That is: 
...
**"Correlation is not causation."** While you've heard this before, let's break it down to internalize this idea. Let's look at the two key concepts in  that statement:

::: aside
I am using the concept of "correlation" broadly for all types of associations. So this is not limited to the strict quantification  of correlation as $r = \frac{Cov_{x,y}}{s_x \times s_y}$ which we will learn soon.
:::


- **Correlation** means that two variables are statistically associated. 
    - A positive association means that when one variable is large (or small) the other is often large (or small).  
    - A negative association means that when one variable is large (or small) the other is often small (or large).


- **Causation** means that changing one variable produces a change in the other (see [Wikipedia](https://en.wikipedia.org/wiki/Causality) for more discussion of "causation".)
...
Correlation is often confused for causation because it's easy to assume that if two things are associated, one must be causing the other. This is especially tempting when the association makes sense to us. But of course, this is wrong. While correlation may hint at causation, a direct cause is neither necessary nor sufficient to generate a correlation.  Take the video in @fig-train  -- an alien might think this man is starting and stopping the train, but clearly he has nothing to do with the train starting or stopping. 

Three mechanisms may underlie a correlation without a causal relationship: Coincidence, Confound, and Reverse causation.   
...
I'm dealing with this issue right now! In natural hybrid zones, white-flowered *parviflora* plants tend to carry less genetic ancestry from *xantiana* (their sister taxon)  than do pink-flowered *parviflora* plants. There are two potential explanations for this observation:   

1. Perhaps, as the  RIL data suggests, white-flowered *parviflora* plants are less likely to hybridize with *xantiana* than are pink-flowered *parviflora*. Thus, white parviflora came from a lineage of white-petalled plants whose petal color helped prevent introgressive hybridization. As such white-flowered plants have less *xantiana* ancestry (pink flowers cause more gene flow).       

2. Alternatively, because all *xantiana* are pink-flowered, while  *parviflora* can be white or pink, it's possible that pink-flowered parviflora plants acquired their pink color from introgressive hybridization with *xantiana* (more gene flow causes pink flowers). 
...
Then we [summarize the chapter](#summarizing_associations_I), present [practice questions](#summarizing_associations_practice-questions_I), a [glossary](#summarizing_associations_glossary-of-terms_I), a review of [R functions](#summarizing_associations_key-r-functions_I) and [R packages](#summarizing_associations_r-packages-introduced_I) introduced, and [present additional resources](#summarizing_associations_additional-resources_I).  


In the next chapter we will continue our study of associations as we will investigate relationships between continuous variables!