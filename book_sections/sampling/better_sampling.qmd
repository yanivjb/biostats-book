...
- **A large sample minimizes the extent of sampling error.**  Sampling error is unavoidable, but it can be minimized -- the larger the sample the less sampling error and the larger the sample the less sampling error (see the previous section on [Sampling error](#sampling_error)).  Of course, larger samples come at a cost of time and energy, and each additional sample becomes less useful as our sample size gets large. So later in the term we will consider how to balance these issues in planning for a good sample size. 
