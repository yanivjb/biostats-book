## • 8. Sampling Better {.unnumbered #better_sampling}
...
3.  Have some familiarity with good enough sampling approaches when a random sample cannot be obtained. 
...
**The ideal sample is a large random sample.**   

- **A large sample minimizes the extent of sampling error.**  Sampling error is unavoidable, but it can be minimized -- the larger the sample the less sampling error  (see the previous section on [Sampling error](#sampling_error)).  Of course, larger samples come at a cost of time and energy, and each additional sample becomes less useful as our sample size gets large. So later in the term we will consider how to balance these issues in planning for a good sample size. 
...
#| column: margin  
#| echo: false
#| fig-cap: "Rollover text: *In some cases especially behavioral ones, creating a believable placebo is impossible, highlighting a key limitation of this experimental design principle.* From [xkcd](https://xkcd.com/1462/). See explanation at [explain xkcd](https://www.explainxkcd.com/wiki/index.php/1462:_Blind_Trials)."
...
Of course, while getting a random sample is ideal, it's not always plausible. Your best alternatives include **Systematic Sampling** e.g. stretching a 100-foot transect line across the hillside and sampling the single plant closest to the line (and within x square feet) every 5 feet. This disciplined approach prevents you from only sampling the convenient spots or the big, showy patches of flowers.
...
There are also things we can do to minimize bias and non-independence in experiments. As stated previously, when conducting human trials a double blind study is best. Similarly, when working in non-humans (plants, animals, bacteria etc) it's best if the observer is "blind" to the treatment.  However, in some cases "blinding" is impossible or impractical -- e.g. how can you watch pollinators and not see petal color? (see also @fig-xkcd_sex).
