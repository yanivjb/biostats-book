## • 8. Sampling summary {.unnumbered #sampling_summary}
...
Please interact with this custom chatbot ([**ChatGPT version here**](https://chatgpt.com/g/g-687e94ec840c8191ae844f3030e9adb4-sampling). [**Gemini version**](https://gemini.google.com/gem/1r6UJEu2uEwSt66cSUZ2RKNI721nDczKG?usp=sharing)) I have made to help you with this chapter. I suggest interacting with at least ten back-and-forths to ramp up  and then stopping when you feel like you got what you needed from it. 
:::
...
#| fig-alt: "A histogram showing the distribution of lengths for all human genes. The figure shows that most human genes are relatively short (median around 3kb), with a few genes being exceptionally long."
library(ggplot2)
library(readr)
...
#| fig-alt: "The distribution of estimated mean human gene length from 1000 replicates of fifty genes. The x-axis is gene length (kb) from 0 to 10. The data is concentrated between 3 and 6 kb, with the tallest peak centered around 4 kb, and a second smaller peak near 5.5 kb. A vertical dashed red line showing the actual population mean is shown at approximately 2.6 kb, to the left of the main cluster of data."
...
**Q6)** Explain your reasoning for the answer above. Any guesses on how I could have gotten this so wrong? How could this be fixed? <html><textarea rows="6" cols="50" placeholder="Your answer here..."></textarea></html>      
...
**Q7)** Why is the range of values so much smaller in @fig-genes-2 than in @fig-genes?   `r longmcq(c( "Sampling bias","Non-independence", "Sampling error", answer = "Parameter estimates usually have a tighter distribution than all observed values."))`
...
**Q10)** For the sampling distribution for samples of size n in @fig-genes-3, approximately what proportion of samples have a mean greater than the population mean? 

- **n = 5:** `r mcq(c( "Way less than 0.01", "about 0.01", "about 0.10", "about 0.25", answer = "about 0.50"))`.  
- **n = 500:** `r mcq(c( "Way less than 0.01", "about 0.01", "about 0.10", "about 0.25", answer = "about 0.50"))`.
...
- **n = 5:** `r mcq(c( "Way less than 0.01",  "about 0.01", answer = "about 0.10", "about 0.25", "about 0.50"))`.  
- **n = 500:** `r mcq(c(answer =  "Way less than 0.01", "about 0.01", "about 0.10", "about 0.25",  "about 0.50"))`.
...
This is the entire population. We have a parameter known without sampling error, so there is not a standard error.
