# X4. Cause {.unnumbered #causal_inference}
...
1. Explain why $p < 0.05$  does not establish causation.

2. Draw and interpret simple DAGs representing confounds, pipes, and colliders.

3. Distinguish a confound from a mediator from a collider.

4. Explain why statistically adjusting for a confound can recover a causal estimate, while adjusting for a collider can create a spurious one.

5. Use a hypothesized causal structure to decide which variables belong inand out of a multiple regression model.

6. Recognize that randomized controlled experiments remain our strongest tool for establishing cause, and understand what careful reasoning about observational data can and cannot offer in their absence.
...
By forcing us to carefully lay out our causal story, DAGs can show us where potential biases or confounding might exist. As such DAGs can guide us in deciding which variables we need to include or exclude in an analysis. For example, by looking at a DAG, you should be able to decide which variables to include (or exclude) in a regression model.
...
dag.4<-ggdag_canonical(dag.4, layout = "circle",node_size = 12,label_size = 22,text_size = 2.5)+theme_tufte()+
  theme(axis.text = element_blank(), axis.title = element_blank(), axis.ticks = element_blank(), axis.line = element_line(color = "lightgrey"), axis.line.x.top = element_line(color = "lightgrey"), axis.line.y.right =  element_line(color = "lightgrey"))+ labs(title = "D) A collider")
...
- One thousand fish 
- Environmental quality for each fish (normally distributed with mean $\mu = 50$ and standard deviation $\sigma = 5$)
- Fish length is independent of quality  (normally distributed with mean $\mu = 50$ and standard deviation $\sigma = 5$)
...
We see everything that is expected, and our stats back this up. 
...
Here the correct model is to include both causes, excluding one will likely lead to unbiased estimates of the effect of the other, but it will reduce power and increase uncertainty. 
