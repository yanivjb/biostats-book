# 9. Uncertainty {.unnumbered #uncertainty}
...
<article class="drop-cap">In the [previous chapter](#sampling) we sampled from a population many times to build a sampling distribution.  If we had data for the entire population, we would **know the population parameters** and would not need to **estimate them from a sample**.</article><br>       



In the real world we usually have just one sample and we want to learn about the population. This is a major challenge of statistics.   
...
The **sampling distribution** -- a histogram of sample estimates we would get by repeatedly sampling from a population -- allows us to think about the chance deviation between the sample estimate and the population parameter induced by sampling error.    


## Estimation with uncertainty   

Because estimates from samples take their values by chance, it is misleading to present an estimate without also describing the uncertainty around it. 
