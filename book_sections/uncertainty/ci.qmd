## • 9. Confidence Intervals  {.unnumbered #ci}
...
Confidence intervals offer a different way to describe uncertainty than the standard error. While the SE is a single number representing expected error (e.g. a standard error of 2.37), a **Confidence Interval provides a range of plausible values for the true population parameter**. 

Confidence intervals can be tricky to interpret and explain. I think about a confidence interval as a net that does or does not catch the true population parameter. If we set a 95% confidence level, as is statistical tradition, we expect ninety-five of every one hundred 95% confidence intervals to capture the true population parameter.
...
```{r}
#| code-fold: true
#| code-summary: "Code for making a very involved histogram of the bootstrap distribution and the 95% confidence interval."
#| eval: true
#| message: false
#| warning: false
#| label: fig-bootci
...
A good plot is clear and honest. So, be sure to include both raw data, an uncertainty in your parameter estimates in your plots. ggplot makes this easy with  [`stat_summary()`](https://ggplot2.tidyverse.org/reference/stat_summary.html) function. 
...
::: fyi
For years I taught intro biostats with Base R tools. One big reason I switched to ggplot and the tidyverse was how easy ggplot2 makes it to add error bars. This took loads of effort in the Base R days.
:::
