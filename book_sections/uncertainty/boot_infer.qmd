## • 9. Bootstrapping w/`infer`  {.unnumbered #boot_infer}
...
gc_rils <- ril_data |>
  dplyr::filter(location == "GC", !is.na(prop_hybrid), ! is.na(mean_visits))|>
...
**Motivating Scenario:** You can  bootstrap yourself with [`slice_sample()`](https://dplyr.tidyverse.org/reference/slice.html), but it's tedious. You would rather use a simple R package that works for many different estimates and does the hard bits for you.

**Learning Goals: By the end of this subsection, you should be able to:**

1. **Use  [`infer`](https://infer.netlify.app/)'s  [`specify()`](https://infer.netlify.app/reference/specify) → [`generate()`](https://infer.netlify.app/reference/generate) → [`calculate()`](https://infer.netlify.app/reference/calculate) pipeline** to build a bootstrap distribution for a statistic.

2. **Use the [`sd()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/sd.html) and [`infer::get_ci()`](https://infer.tidymodels.org/reference/get_confidence_interval.html?q=get_ci#aliases) functions** to find a bootstrap standard error and confidence interval from that distribution.

3. **Apply this procedure to numerous summary statistics** including a mean, a difference in means, a correlation etc... to empower yourself to quantify uncertainty in many types of summaries.
...
We previously generated a bootstrap distribution with [`slice_sample()`](https://dplyr.tidyverse.org/reference/slice.html). But this required some work on our part. As with much in R there are specialized packages that bootstrap for us. I like these packages because they make it easier to use, harder to mess up, and often coded more efficiently. 


Here, I show you how to bootstrap with the [`infer`](https://infer.netlify.app/) package. While our first example of bootstrapping a single mean will look similar to our manual approach, you will quickly see how the [`infer`](https://infer.netlify.app/) package's consistent grammar makes bootstrapping more complex statistics (e.g.  a difference in means or a regression slope) surprisingly straightforward.
...
`r hide("Show the entire pipeline with infer tools")` 
...
I could code this myself, but it's tedious. [`infer`](https://infer.netlify.app/) makes it straightforward. The code below uses the formula syntax `prop_hybrid ~ petal_color` (We will see more of this when we dive into [linear_models](#linear_models)) inside [`specify()`](https://infer.netlify.app/reference/specify) to indicate our interest in this relationship. We then tell R to [`generate()`](https://infer.netlify.app/reference/generate) the bootstrap distribution and [`calculate()`](https://infer.netlify.app/reference/calculate) to find the `"diff in means"` for each bootstrap replicate:
...
We can similarly use infer to find the bootstrap distribution of the association between two continuous variables.  For example, we can  quantify the uncertainty in our estimated correlation of `r gc_rils|> dplyr::filter(!is.na(prop_hybrid) & ! is.na(petal_area_mm))|> summarise(cor = cor(prop_hybrid, petal_area_mm)) |>pull() |> round(digits = 3)` between proportion hybrid seed and petal area in $mm^2$ (@fig-slope) as shown in the code below. 

```{r}
#| column: margin
# Estimate the correlation
gc_rils |>
  dplyr::filter(!is.na(petal_area_mm)) |>
  summarise(correlation = cor(petal_area_mm, prop_hybrid) )
```

```{r}
boot_cors <- gc_rils                      |>
  dplyr::filter(!is.na(petal_area_mm))             |>
  specify(prop_hybrid ~ petal_area_mm)      |>        # The relationship we're looking into
  generate(reps = 5000, type = "bootstrap") |>        # Bootstrapping
  calculate(stat = "correlation")                     # Summarizing the bootstrap replicate

summarise(boot_cors , se = sd(stat))                # find SE
get_ci(boot_cors,level = 0.95, type = "percentile") # 95% CI
```
...
So our estimated correlation lies between  `r ((boot_cors |>get_ci(level = 0.95, type = "percentile"))|>unlist() |> round(digits =3)) |> paste(collapse = " and ")`.  This is super helpful, because without quantifying uncertainty this correlation may reflect noise rather than signal. That is, the correlation could disappear in another sample. But because our entire 95% confidence interval excludes zero, we can be confident this association reflects a true correlation greater than zero, rather than a weird sample.
