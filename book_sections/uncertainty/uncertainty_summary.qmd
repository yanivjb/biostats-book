## • 9. Uncertainty summary {.unnumbered #uncertainty_summary}
...
Please interact with this custom chatbot ([**ChatGPT version here**](https://chatgpt.com/g/g-6882957bbbd48191914c8738d011de82-uncertainty-bootstrap-tutor), [Gemini version](https://gemini.google.com/gem/1mPLyeYxf_qVIoajXfeMfCbIzVaHnQuIs?usp=sharing)) I  made to help you with this chapter. I suggest interacting with at least ten back-and-forths to ramp up  and then stopping when you feel like you got what you needed from it. 
...
The **Sampling distribution** is the foundation idea for thinking about uncertainty. It's the distribution of sample estimates, and it's what allows us to think about the nature of sampling error.   
...
`r hide("Why Standard deviation is wrong.")`
...
**Q5)** You actually know the population parameter. A bunch of friends sample randomly from this population, and calculate 95% confidence intervals. What proportion of these confidence intervals will catch the true parameter? (check all that apply). `r mcq(c("There is no probability about this, it either did or it did not", answer = "95%", "It depends on the sample size" , "It depends on the sample standard deviation"))`
...
**The faithful data:** Old Faithful is meant to erupt pretty regularly, how regularly is this? Let's look into it by estimating our uncertainty in the mean waiting time in the faithful data set in R.

First, let’s take a glimpse of the [`faithful`](https://stat.ethz.ch/R-manual/R-devel/library/datasets/html/faithful.html) data.
...
:::exercises
**Q6)** The distribution of **waiting** times at Old Faithful is ___ `r mcq(c("Unimodal", answer = "Bimodal", "Symmetric"))`
...
**Q9)** Following up on the previous question, the **lower bound** of the **99% confidence** interval of mean waiting time is `r fitb(68.8, num = TRUE, tol = .3)`

`r hide("Explanation for Q9")`
...
This greater confidence comes at a cost. A 99% confidence interval is wider and less precise than a 95% interval calculated from the same data.   
...
* **Confidence Level**: The percentage (e.g., 95%) that describes how often a confidence interval, if you repeated the sampling process many times, would be expected to capture the true population parameter.  
...
[Why you are not allowed to say that your 95% confidence interval contains the true parameter with a probability of 95%](https://www.the100.ci/2024/12/05/why-you-are-not-allowed-to-say-that-your-95-confidence-interval-contains-the-true-parameter-with-a-probability-of-95/). by [Julia M. Rohrer](https://juliarohrer.com/), from. her blog - [The 100% Confidence Interval](https://www.the100.ci/).
...
- This [webapp](https://www.zoology.ubc.ca/~whitlock/Kingfisher/CIMean.htm) from @whitlock2020 is great for thinking about confidence intervals. It's math-based, not bootstrap-based, but the concepts and ideas are the same.
