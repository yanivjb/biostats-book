...
We realized that this was because the coefficient of `0.2373` for `petal_colorwhite` describes the difference between its expected value at the intercept (i.e. `petal_area_mm` = 0, @fig-interactioncombo B) and that of the pink-petaled RILs. This prediction has no biological application, as no plants have petals of size zero $mm^2$. 
...
Of course, no statistical description is complete without a measure of uncertainty. Again we can accomplish this with the [`emmeans`](https://rvlenth.github.io/emmeans/) or `modelbased` packages. Here I use [`estimate_means()`](https://easystats.github.io/modelbased/reference/estimate_means.html) function in the [`modelbased`](https://easystats.github.io/modelbased/) package, because I find it easier to code. 


So to find uncertainty about the point estimates of the proportion of hybrid seeds for pink- and white-petaled RILs with petal area of 45 and 85 $mm^2$, we type: 
...
*We tested the contribution of petal color, petal area and their interaction to the proportion of hybrid seeds on a RIL. Using Type II sums of squares, we found significant associations between the proportion of hybrid seeds and petal color ($F_{1,398}$ = 80.23, p < 2.2e-16), petal area ($F_{1,398}$ = 28.2, p < 2 e-7), and their interaction ($F_{1,398}$ = 24.4, p < 2 e-6). This interaction reflects the sharp increase in the proportion of hybrid seed with petal area in pink-, but not white-petaled RILs (compare slopes of 0.0075 -- 95% CI: {0.0054 : 0.0095}  and 0.0007 -- 95% CI: {-0.0010 : 0.0025}, respectively).*
