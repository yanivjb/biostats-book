## • 16. Two t Summary {.unnumbered #2t_summary}
...
Please interact with this custom chatbot ([**link here**](https://chatgpt.com/g/g-68c3a28f94848191b4c91bcd61cdd87d-two-sample-t-tutor)). I have made it to help you with this chapter. I suggest interacting with at least ten back-and-forths to ramp up  and then stopping when you feel like you got what you needed from it. 
...
*To evaluate this idea @swierk2019, identified a male's top choice out of two female wood frogs and then had them mate with the preferred or unpreferred female, and counted the number of hatched eggs.*
...
:::exercises
**Q1.** Complete the code above, what pattern do you see? `r longmcq(c(answer = "Similar numbers (or slight differences) of hatched eggs in preferred and nonpreferred treatments", "Way more hatched eggs in the preferred treatment", "Way more hatched eggs in the nonpreferred treatment"))`
:::
...
`r longmcq(c("A) Both treatments combined.", answer =  "B) Separate data by treatment.", "It depends, you should integrate information from both.",  "Neither plot is appropriate"))`
...
**Q3.** Consider the output of the code above. Should you feel comfortable assuming equal variance?  `r longmcq(c(answer= "Yes", "No", "Maybe","I don't know (I need to run a statistical test to evaluate this assumption)","Can you repeat the question?"))`

`r hide("Click here for my justification.")`

The variance is not equal -- the variance in the "preferred" group is 1.20 times greater than the non-preferred group. But remember that we don't worry about differences in variance until the difference is more than 4X. 
...
`r longmcq(c("Not worth reporting (Cohen's D is less than 0.01)", "Tiny	(Cohen's D is between 0.01 and 0.20)",answer =  "Small	(Cohen's D is between 0.20 and 0.50)", "Medium	(Cohen's D is between 0.50 and 0.80)", "Large	(Cohen's D is between 0.80 and 1.20)", "Very large	(Cohen's D is between 1.20 and 2.00)", "Huge	(Cohen's D is greater than 2.00)"))`
...
**Q6.**  From this Cohen's D value, we conclude that:
...
**Q9.** Which values are NOT IN the 95% confidence interval of the difference (i.e. nonpreferred - preferred) select all that apply:
`r longmcq(c(answer = "-92", "-42","2","52","102","152",answer ="202",answer = "252"))`
...
**Q10)** The output of the code above shows a p-value of $\approx 0.30$, so we fail to reject the null hypothesis. The output also shows a t-value of $\approx 1$. 

**Q10A)** If all I knew was that $t \approx  1$, but did not know the degrees of freedom, would I be able to reject the null at $\alpha = 0.05$? `r mcq(c("Yes", answer = "No","Can't tell you without the degrees of freedom."))`

**Q10B)** Say for a different dataset I knew that $t = 2$ (rather than one) but did not know the degrees of freedom, would I be able to reject the null at $\alpha = 0.05$? `r mcq(c("Yes","No",  answer =  "Can't tell you without the degrees of freedom."))`


`r hide("Click for an explanation")` 

When the absolute value of t is less than 1.96 we will fail to reject the null hypothesis at $\alpha = 0.05$, regardless of the degrees of freedom.

Whenever  the absolute value of t is greater than 1.96 we may or may not fail to reject the null hypothesis at $\alpha = 0.05$, depending on the degrees of freedom. 

If we have more than five degrees of freedom, we will reject the null when the absolute value of t is greater than 2.5.  
...
`r hide("This may be controversial. Click for my thinking")` 

In practice, it is very unlikely that two population means are exactly equal. Even if the true difference is small, it is rarely zero. Statistical tests help us decide whether we have enough evidence to detect that difference. Not whether it exists at all.

**This is why we do not accept the null - we simply fail to reject it.**
...
- [Khan Academy: Two sample t-test](https://www.khanacademy.org/math/ap-statistics/xfb5d8e68:inference-quantitative-means/two-sample-t-test-means/v/two-sample-t-test-for-difference-of-means).    
