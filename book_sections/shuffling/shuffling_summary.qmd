## • 11. Shuffling summary {.unnumbered #shuffling_summary}
...
The null hypothesis is usually something like "the true population parameter of my response variable is the same for all values of my explanatory variable" (i.e. independence). Permutation approximates the null sampling distribution by (1) randomly swapping values of the explanatory variable in our data, and (2) calculating a test statistic summarizing the association in this randomly permuted dataset (3) repeating this many times to generate a null sampling distribution. We then find a p-value by finding the proportion of this null distribution that is as or more extreme than the observed test statistic. R's [`infer`](https://infer.netlify.app/) package has a bunch of handy tools to make permutation easier! 


:::aside
<h2>We Swapped Group Labels 10,000 Times...<br><br>You Won’t Believe What Happened Next!</h2> -- Permutation clickbait.
...
Please interact with this custom chatbot ([**link to ChatGPT chatbot here**](https://chatgpt.com/g/g-6890ceababb881919cf0b449f58b30b1-permutation-tutor), [**link to Gemini chatbot here**](https://gemini.google.com/gem/1mCinJo-YX-S_ZieHkGgSzr3TRnqWeJbL?usp=sharing)) I created to help you with this chapter. I suggest interacting with at least ten back-and-forths to ramp up  and then stopping when you feel like you got what you needed from it. 
...
**Q4)** For a **bootstrap** focusing on differences in mean body mass between male and female penguins, are the following statements True or False?       
...
- *4c:* The bootstrap distribution of the sex difference in body mass will be centered around the original estimate. `r torf(TRUE)`.  
...
The `penguins` dataset (loaded) below --  a fun dataset provided by the `palmerpenguins` package contains some basic data on Adelie penguins and other penguin species. The code below loads in the packages and data we need to explore these guys! 
...
The code below builds on the quick [`glimpse()`](https://dplyr.tidyverse.org/reference/glimpse.html) of the data (above) by demonstrating how to [`filter()`](https://dplyr.tidyverse.org/reference/filter) the data and get numeric summaries by group. This refresher should help prepare you for the next tasks
...
**Q6)** The mean body mass of *male* Adelie penguins from Torgersen island  is `r fitb(answer = 639, tol = 12, num = TRUE)` g heavier than females.
...
```{webr-r}
adelie_torgersen |># a pre-filtered dataset
    specify( A ~ B) |>
...
**Q11)** Modify the code above to find the permutation-based p-value (from 5000 permutations). How would you report it?  `r mcq(c("0", answer = "< 3/5000", "NA","0.02","Between 0.02 and 0.05", ">0.05", "1/Inf"))`
...
`r hide("What the he🏒🏒? Why is the answer from R wrong? A-hole")`
If you did this right, you got a p-value of 0 with the warning: 

<p style="color:red;">Warning: Please be cautious in reporting a p-value of 0. This result is an approximation based on the number of *reps* chosen in the *generate()* step. ℹ See *get_p_value()* (*?infer::get_p_value()*) for more information. </p>
...
**Q13)** We can't **KNOW** if the null hypothesis is true or false, but which do you think is far more likely? `r longmcq(c("Null hypothesis is true",answer = "Null hypothesis is false", "The null hypothesis is equally likely to be TRUE or FALSE"))`
...
## 📊 Glossary of Terms {#shuffling_summary_glossary-of-terms} 

:::glossary

* **Permutation (Permutation Test)**: A null sampling distribution we build by randomly shuffling the relationship between individuals'  explanatory and response variables. This will break any real association, and reveal the  expected range of estimates if there were no actual association.
* **Permuted (Null) Distribution**: The distribution of a test statistic (e.g., difference in means) computed across many permuted datasets. This approximates the sampling distribution of the statistic under the null.
* **p-value (permutation-based)**: The proportion of permuted (null) test statistics that are as or more extreme than the one actually observed.
* **Structured (Blocked) Permutation**: A modified permutation procedure that shuffles labels only *within* meaningful subgroups (e.g., within each year or site) so the null distribution respects non-independence in the data instead of ignoring it.
...
There are no new packages, but we continue to use **[`infer`](https://infer.netlify.app/):** this time for permutation. 

:::

---

## 🛠️ Key R Functions{#shuffling_summary_key-r-functions}  
