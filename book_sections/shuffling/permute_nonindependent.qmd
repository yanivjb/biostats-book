## • 11. Structured Permutation {.unnumbered #permute_nonindependent}
...
**Motivating Scenario:** We want to run a permutation test, but we recognize that the data are not independent. We know that independence is an assumption of the standard permutation test. What to do? Here we will learn to permute in a way that respects the structure of your data.
...
This plot reveals a classic statistical problem. It’s possible that one treatment was better within each year, but this pattern is hidden in the overall data because the treatments were not balanced across the 'good' and 'bad' years.  If we permute treatment labels across all observations, ignoring year, our null distribution will be incorrect because it mixes treatment effects with year effects.

## Permuting with structure

To address this non-independence, we need to shuffle treatment labels **within each year** rather than across the entire dataset. This ensures that any differences we detect are not confounded by the year of data collection. With more complex datasets, this issue can become even trickier, but the same principle applies: you need to preserve the structure of the data by permuting within appropriate groups.



To do this, we will split our data into different years, permute within each year, and then combine the permuted data to build a null sampling distribution reflecting the structure in the data. Unfortunately `infer` can only get us so far. So instead, we'll take matters into our own hands!
...
structured_perm <- replicate(1000, simplify = FALSE, {
  ...
  frogs |>
    ...
    # Change data from long to wide format, with separate columns for the 'preferred' and 'nonpreferred' means.
...
structured_perm |>
  ...
  labs(x = "Difference in means", fill = "As or more extreme", title = "Structured permutation distribution")+
...
But because our null distribution is no longer centered at zero (see @fig-permstructure), we redefine "extreme" as the distance away from the mean of the null sampling distribution (rather than the distance away from zero). We then add up the area in the right and left tails to find our p-value.
