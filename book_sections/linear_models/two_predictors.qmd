...
With the estimates above, we can  return to our general linear model equation to find conditional means:  $$\widehat{\text{Prop hybrids}}_i = b_0 + b_1   \times  \text{PETAL AREA}_i +  b_2 \times \text{WHITE}_i$$ $$\widehat{\text{Prop hybrids}}_i =  -0.0914 + 0.00576  \times  \text{PETAL AREA}_i - 0.2392   \times \text{WHITE}_i$$
...
y_1_pred <- -0.0914 + 0.00576 * 44 - 0.2392 * 1       
...
- Find the predicted proportion of hybrid seed for individual $i=3$ `r fitb(num = TRUE, 0.206, tol = 0.002)`.  

- Find the residual proportion of hybrid seed for individual $i=3$ `r fitb(num = TRUE, 0.044, tol = 0.003)`.
...
-0.0914 + 5.76e-3 * 51.7 #   no b2 because it is pink  

# Residual for ind i = 3
0.25 - (-0.0914 + 5.76e-3 * 51.7)
```
`r unhide()`
