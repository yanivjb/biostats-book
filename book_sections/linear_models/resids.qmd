## • 13. Residuals {.unnumbered #resids}
...
Take, for example, individual 14 ($i = 14$, green point in the plot below), where five of eight (0.625) of its seeds are hybrids:  
...
- **Its residual value, $e_i = y_i - \hat{y}_i$,** is the difference between the proportion of its seeds that are hybrids and the proportion of all seeds that are hybrids, which is $0.625 -`r mean(gc_rils$prop_hybrid , na.rm = TRUE)|> round(digits = 3)` = `r 0.625 - mean(gc_rils$prop_hybrid , na.rm = TRUE)|> round(digits = 3)`$. Scroll over the fourteenth data point in @fig-plotly to verify this. 
...
While you should consider the shape of our data when summarizing it, a common criterion for the "best" estimate is the one that minimizes the sum of squared residuals. @fig-minimize_ss_a shows that of all proposed means for the proportion of hybrid seeds set at GC, the arithmetic mean minimizes the sum of squared residuals. This is always the case!
