## • 13. Linear model summary {.unnumbered}
...
**Q6)** How many mm longer are Chinstrap bills as compared to Gentoo bills in this dataset?  `r mcq(c("10.042","8.713",answer = "1.329","not enough information"))`
...
# This is about understanding concepts
# So do not run linear models here.
# Calculate correlation, r, as: r = cov(x,y) / (sd(x)*sd(y))

# Calculate slope as: b = cov(x,y) / var(x)

# Calculate intercept as: a = mean(y) - b * mean(x)
...
**Q13)**  According to the model, what is the predicted bill depth for a penguin with a 500 mm long bill? `r fitb(-10000000000000)`
...
library(ggplot2)
library(palmerpenguins)
# Work here! You will build a linear model

```

:::exercises

**Q14)** In @fig-penguin there is a male penguin with a bill that is about 56 mm long (the second most extreme right point). Visually approximate its residual.`r fitb(.6,tol=.15)`
...
- **Slope (`b₁`)**: The expected change   $\hat{y}_i$ with change in the predictor.     
   ...
   - *For binary predictor:* The difference in the mean of non-reference and reference level.  
...
- **Residual Standard Deviation**: A measure of typical residual size — how far off predictions tend to be $\sqrt{\frac{\sum e_i^2}{n-k}}$, where $k$ is the number of parameters estimated in our model. For a simple linear regression $k=2$, as we estimate the slope and the intercept.
...
- **[`sigma()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/sigma.html)** Find the residual standard deviation from a linear model.  
