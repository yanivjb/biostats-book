## • 13. Categorical predictor {.unnumbered}
...
To start, let’s focus on petal color, because perhaps white petals do not attract pollinators and consequently may produce fewer hybrids. 

As a vaguely stated  linear model, this is: 

$$\hat{y}_i = b_0 + x_i \times b_1$$ In general words:    $$\text{Prediction} = \text{Intercept} + \text{``Effect'' of thing} \times \text{value of thing}$$ 

In the specific case of petal color, and assuming pink is the "reference level":
 

$$
\text{Predicted prop hybrid} = \text{Intercept} + \text{``Effect'' of white} \times
\begin{cases}
1, & \text{if the petals are white},\\
0, & \text{if the petals are pink}.
\end{cases}
$$

where "Effect" of white is the difference between the mean proportion of hybrid seeds on white- and pink- flowered RILs. 


Now let's build and interpret this linear model!! 
...
$$\text{Predicted prop hybrid | White flower} = 0.2598  - 0.2277 \times 1 = 0.0322$$
