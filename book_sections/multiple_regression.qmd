# 19. Multiple Predictors {.unnumbered #multiple_regression}
...
ASD, the (new to us) variable quantifies the distance between the pollen-producing anther and the ovule-bearing stigma parts of a flower (@fig-asd). The logic of this hypothesis is that having male and female parts close to one another can encourage self-fertilization before there is an opportunity for hybridization. 
...
Multiple regression is lots of fun! When first learning about it, it is easy to get carried away and put every variable in the data set into the model. Please do not do this, as overly large models can be hard to interpret,  can cost us statistical power, can yield surprising or unstable regression coefficients, and may misatribute patterns to the wrong variable.  
