...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/1TtCulwRAn3CTKZ6Rnx6WdzSNCNELcEuz/view?usp=drive_link) &nbsp;·&nbsp; 🎧 [Listen](../audio/intro_to_r/Intro2R.mp3)
:::
...
<article class="drop-cap">Modern data analysis and visualization usually requires a computer a computer. To make such analyses reproducible, a scripting language like R helps a lot. While there are "point and click" programs that you can use for stats, these are often expensive and/or limited. So our goal in this section is to get everyone on board with using R for stats.</article>
...
Of course, there is a lot covered under "getting on board with using R for stats." So in the next six sections our immediate goals are:   
...
#| echo: false
#| label: "fig-r_mindset"
#| fig-cap: "Video of Yaniv's hints for successful R coding."
...
- **You don't need to memorize anything.** You have access to dictionaries, translators, LLMs etc etc. That said, these tools are more useful the more you know.  
...
- **[Getting up and Running](#getting_started).** This section introduces RStudio, math in R, vectors, variable assignment, using functions, r packages, loading data (from the internet), and data types. There is a lot here! 


- **[Intro to ggplot](#intro_to_ggplot).** The ggplot package allows us to make nice plots quickly. We will get started understanding how ggplot thinks, and introduce the wide variety of figures you can make. Later in this term we will make better figures in [ggplot](https://ggplot2.tidyverse.org/).


- **[Reproducible science](#reproducible_science).** We consider how to collect data, and store it in a folder and write reproducible R pipelines.

- **[Data in R](#data_in_R).**   Here we continue on our introduction to R. We first introduce the concept of tidy data, and introduce the capabilities of the tidyverse package, [dplyr](https://dplyr.tidyverse.org/).   

- We then use R to summarize our data, both for a single variable ([Chapter 5](#univariate_summaries)), and for combinations of variables ([Chapter 6](#associations) and [Chapter 7](#associations_II)). 