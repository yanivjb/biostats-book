## •  18. ANOVA Example  {.unnumbered  #anova_example}
...
- Generate and interpret ANOVA output in R with the  [`aov()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/aov.html) or [`lm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/lm.html) pipelines, and relate the anova output to the idea of variance partitioning.
...
We can use the logic illustrated in @fig-devs to calculate the relevant sums of squares, and the proportion of variance explained by the model, $r^2$: 

:::aside
We often see $SS_\text{model}$ and $SS_\text{error}$ calculated as means or variances within groups weighted by the  size of each group. Those definitions are nice for rapid calculation of an ANOVA from summary statistics, but I prefer this observation-centered approach. Of course, because both approaches give the same answer, do what you feel.
:::
...
:::aside
[`pf()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Fdist.html) looks up our F value on the F distribution with df1 = df_model (aka df_groups), and df_2 = df_error  (aka df_resid). Recall that F is a one-tailed test -- we only want the upper tail (`lower.tail = FALSE`)
:::
...
Of course R can do these calculations for us, with either the [`aov()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/aov.html) `|>` [`summary()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/summary.aov.html) pipeline, or the [`lm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/lm.html) `|>` [`anova()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/anova.html) pipelines. Both approaches yield the same result as our manual calculation, above.



### [`aov()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/aov.html) `|>` [`summary()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/summary.lm.html)
...
###  [`lm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/lm.html) `|>` [`anova()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/anova.html)
