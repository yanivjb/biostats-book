...
- Generate and interpret ANOVA output in R with th  [`aov()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/aov.html) or [`lm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/lm.html) pipelines) and relate it to the  variance partitioning.
...
:::aside
We often see $SS_\text{model}$ and $SS_\text{error}$ calculated as means or variances within groups weighted by the  size of each group. Those definitions are nice for rapid calculation of an ANOVA from summary statistics, but I prefer this observation-centered approach. Of course, becasue both approaches give the same answer, do what you feel.
:::
