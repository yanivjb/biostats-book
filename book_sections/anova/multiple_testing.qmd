## •  18. Multiple testing problem  {.unnumbered  #multiple_testing}
...
```{r}
#| echo: false
#| fig-cap: "All six pairwise comparisons of mean admixture proportion among four *Clarkia xantiana parviflora* hybrid zone populations. This presentation implicitly sets up six separate null hypotheses to test. Compare this to the figure in the first section, which showed data from all four sites together in one plot."
#| fig-alt: "Six panels show pairwise comparisons of mean admixture proportion among four Clarkia populations (SR, S22, S6, SM). Each panel contains two colored violins with overlaid points and black 95 percent confidence intervals. Some pairs appear different, others overlap, but all share the same vertical scale (ranging from zero to one tenth)."
#| label: fig-sixcomps
#| fig-height: 2.5
library(patchwork)
...
#| fig-alt: "Each panel in this xkcd comic shows an announcer reporting no link between a jelly bean color and acne, with p > 0.05 — except one panel showing green jelly beans with p < 0.05 highlighted in red. The final panels show newspapers proclaiming \"Green jelly beans linked to acne!\" despite all other results being null. The comic satirizes how multiple comparisons can produce spurious significance."
