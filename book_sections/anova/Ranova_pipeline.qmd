## •  18. R ANOVA pipeline  {.unnumbered  #Ranova_pipeline}
...
Here's a brief review of how to complete all the bits of an ANOVA in `R`. To do so, let's examine differences in `Sepal.Width` by `Species` in the `iris` dataset.  
...
We saw we can do this with the [`aov()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/aov.html) `|>` [`summary()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/summary.lm.html) pipeline, or the  [`lm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/lm.html) `|>` [`anova()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/anova.html) pipeline. Here I do the latter.  Either way, our answers match those that we derived above:
...
The [`tidy()`](https://broom.tidymodels.org/reference/tidy.lm.html) function in the [`broom`](https://broom.tidymodels.org/index.html) package makes this output tidy and shows our p-values are identical!
...
As above there are a few ways to do this, and, as above we stuck to the [`lm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/lm.html) framework.
...
We see that all groups differ significantly from one another. So call one significance group "a", another significance group "b", and the third significance group "c". It doesn’t matter which ones which—just that they’re all different. These are arbitrary labels. For more complex cases, R can find significance groups for us:
