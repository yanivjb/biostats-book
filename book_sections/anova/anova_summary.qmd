## • 17. ANOVA summary {.unnumbered #anova_summary}
...
Links to: [Summary](#anova_summary_chapter-summary). [Chatbot tutor](#anova_summary_chatbot_tutor).  [Questions](#anova_summary_practice-questions). [Glossary](#anova_summary_glossary-of-terms). [R packages](#anova_summaryR_packages_introduced). [R functions](#anova_summary_key-r-function). [More resources](#anova_summary_additional-resources).
...
**Q10)** According to the post-hoc test, if `female` is in significance group, "a", which group(s) is `intact male major` in? `r mcq(c("a only", "b only", "c only", "a,b", "a,c",answer = "b,c","a,b,bc"))`
...
- [`broom`]([`broom`](https://broom.tidymodels.org/index.html): A collections of functions to tidy the outputs of a linear model,  including; `tidy()`, `augment()`, and `glance()`.     
- [`multcomp`](https://multcomp.r-forge.r-project.org/). A packge with numerous tools for multiple comparisons, including general linear hypothesis testing (`glht()`) and functions to generate significance groupings (`cld()`).    
