# 11. Shuffling {.unnumbered #shuffling}
...
3.  Use `R`'s [`infer`](https://infer.netlify.app/) package to permute data to determine if two sample means come from different (statistical) populations.
...
#| fig-alt: "Animated GIF showing a group of dancers with the text *Everyday I’m shuffling* overlaid. This visual can be used as a playful reference to the process of shuffling data during permutation tests in statistics, where the data is repeatedly randomized to simulate a null hypothesis."
#| column: margin
#| echo: FALSE
#| label: fig-lmfao
...
But how do we find the null sampling distribution? One way, known as **permutation**, generates a null distribution directly from the data by randomly shuffling the explanatory variable labels across observations. By breaking the link between variables and reshuffling many times, permutation simulates the null distribution because any observed differences or associations in the permuted data must be due to random chance because the relationship between variables has been broken. We can then generate a p-value by comparing our test statistic to this empirically generated (permuted) null distribution.

Unlike mathematical tricks we will cover later, permutation makes very few assumptions. The most critical assumptions are that samples are random, independent, collected without bias, and that the null hypothesis assumes no association.  What's more, fancier permutation methods can accommodate some assumption violations (as we will see in the [Structured Permutation section](#permute_nonindependent)). But more advanced permutaiton require more advanced thinkjing and coding so that we can shuffle appropriately. 
...
5.  Calculate the test statistic on this permuted dataset.
6.  Repeat steps 4 and 5 many times to generate the null sampling distribution.
7.  Calculate a p-value as the proportion of permuted values that are as or more extreme than the test statistic observed in the real data.  
...
To give you a quick break from *Clarkia*, and perhaps more importantly, to set up a groan-inducing pun we will move from plants to amphibians! But we are still concerned with mating and its implications. We will use this system to go through the steps above (and more) as follows: 
  ...
  * [The next section](#the_frogs) introduces the biological system, scientific hypotheses and the dataset. In doing so, we also go over steps 1-3 above. This section will also review key concepts of bootstrapping and uncertainty! 
  * [In the section after that](#permute) we generate the null sampling distribution and calculate a p-value with the [`infer`](https://infer.netlify.app/) package thereby addressing steps 4-7 and interpret and write up the results (steps 8-9)!
...
:::note
An exact permutation tests all possible shuffles of the data to get an exact p-value. However this is computationally infeasible for all but the smallest datasets. 
