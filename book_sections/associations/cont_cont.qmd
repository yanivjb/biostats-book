...
In the previous section we used covariance to describe associations between two binary variables. We saw that -- for two binary variables -- the covariance measures how much the observed combination of values ($p_{A,B}$) differed from  expectations under independence ($p_A \times p_B$). 
...
:::note
The equation for the covariance should remind you of the equation for the variance. This is because the variance is simply the covariance of a variable  with itself. (i.e $cov_{X,X} = \frac{\sum(X_i - \bar{X})(X_i - \bar{X}) }{n-1}$ = $\frac{\sum(X_i - \bar{X})^2 }{n-1}$ = $var_X$). You can also see this similarity by comparing @fig-cov, above, to  the earlier  [gif depicting variance calculations](../univariate_summaries/summarizing_variability.qmd#fig-ssgif), from our section on [summarizing variability for a refresher](../univariate_summaries/summarizing_variability.qmd#summarizing_variability).
:::
