...
- [`filter()`](https://dplyr.tidyverse.org/reference/filter.html). In the [`dplyr`](https://dplyr.tidyverse.org/) package. Allows you to choose rows to retain based on their values in  a column. Because there is another `filter()`  function,  always specify that you want dplyr's filter. You can do this by:   
   - Using the [`conflicts_prefer()`](https://conflicted.r-lib.org/reference/conflicts_prefer.html) function in the [`conflicted`](https://conflicted.r-lib.org/) package: `conflicts_prefer("filter", winner = "dplyr")`.  OR
