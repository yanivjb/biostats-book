...
2. Take care to ensure that you do not get the wrong answer when filtering your data.   
...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/1jse46PuTj3KWdcn9EmT7tBks6QZS8-bI/view?usp=sharing) &nbsp;·&nbsp; 🎧 [Listen](../../audio/intro_to_r/data_in_R/choose_rows.mp3)
:::
...
@fig-bad is not good. Most of the space is wasted on `NA` values. This waste also obscures the true pattern. We can fix this by removing cases in which we did not record pollinator visits (i.e. when `visits` are `NA`).  Below, I show how to use [`dplyr`](https://dplyr.tidyverse.org/)'s  [`filter()`](https://dplyr.tidyverse.org/reference/filter.html)  function to do this!
