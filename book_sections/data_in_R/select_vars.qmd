### • 4. Prepping data {.unnumbered #select_vars}
...
1. Use [`dplyr`](https://dplyr.tidyverse.org/)'s  [`glimpse()`](https://dplyr.tidyverse.org/reference/glimpse.html) function to get a sense of our data.  
...
- **Peeking at our data** with [`dplyr`](https://dplyr.tidyverse.org)'s [`glimpse()`](https://dplyr.tidyverse.org/reference/glimpse.html) function reveals the name of each column and the type of data in each.  
- **Choosing columns of interest**  with [`dplyr`](https://dplyr.tidyverse.org)'s [`select()`](https://dplyr.tidyverse.org/reference/select.html) function. This lets us focus on a few columns of interest.   
...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/1jse46PuTj3KWdcn9EmT7tBks6QZS8-bI/view?usp=sharing) &nbsp;·&nbsp; 🎧 [Listen](../../audio/intro_to_r/data_in_R/prepping_data.mp3)
:::
...
To deal with data you must know how to refer to it and what type of data it is. The [`glimpse()`](https://dplyr.tidyverse.org/reference/glimpse.html) function provides both the names of columns in a dataset, and the  type of vector in each column (plus the first few values for each):
...
- `location`: The plant's location. The pollinator visitation experiment was limited to two locations  (either `SR` or `GC`), while the hybrid seed formation study was replicated at four locations (`SR`, `GC`, `LB` or `US`).  This should be stored as a `<chr>` (character), and it is!
...
- `petal_color`: The color of the petal (pink or white).  
- `petal_area_mm`: The petal area (in $mm^2$).  This should be a number `<dbl>` (double), and it is!       
- `asd_mm`: The distance between anther (the place where pollen comes from) and stigma (the place that pollen goes to) on a flower. The smaller this number, the easier it is for a plant to pollinate itself. This should be a number `<dbl>` (double), and it is.  
...
I like to test my code before overwriting the old data with the new. The code above was the "testing." Now that we see that our code worked as expected, enter the following. If we fail to do this assignment, `ril_data` is unchanged.
