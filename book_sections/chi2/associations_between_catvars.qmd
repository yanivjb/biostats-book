## • X6. $\chi^2$ Two Cats {.unnumbered #associations_between_catvars}
...
- The correlation:  $r_{A,B}=\frac{Cov_{A,B}}{s_A \times s_B}$. 
...
We can permute with the infer() pipeline. Note that we specify `stat = "Chisq"`, and  `correct = FALSE`. This latter argument calculates the actual $\chi^2$ value. 
