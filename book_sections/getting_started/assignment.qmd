...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/open?id=1tWiFujixLDNoR0CymbfQh4b5IRWdu-qK&usp=drive_copy) &nbsp;·&nbsp; 🎧 [Listen](../../audio/intro_to_r/getting_started/1_Variable_Assignment.mp3)
:::
...
#### **Math on assigned variables**

We can calculate the total number of seeds per plant as follows

```{r}
num_flowers        * seeds_per_flower          # total seeds (for each plant)
```

We could even assign this to a variable: 

```{r}
total_seeds <- num_flowers        * seeds_per_flower  
```
...
We can find the total number of seeds in the sample by summing: 

```{r}
sum(num_flowers    * seeds_per_flower)         # total seeds (overall)
```

Or, using `total_seeds`, we type  `sum(total_seeds)`, and get `r sum(total_seeds)`.
...
- *Variable assignment allows us to easily reuse the information.* For example, typing `mean(num_flowers)` returns the mean flower number, `r mean(num_flowers)`.   
