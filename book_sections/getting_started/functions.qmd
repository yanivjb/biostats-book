...
If you don’t specify them all, R will either tell you to provide them, or assumes default values. For example, the `log` function defaults to the natural log (base *e*), so  `log(1000)` returns `r log(1000) |> round(digits = 3)`. If you want the logarithm with base 10, you need to specify it explicitly as `log(1000, 10)`, which returns `r log(1000, 10)` (@fig-args **A**). Note that argument order matters: `log(10, 1000)` returns `r log(10, 1000)`, while `log(1000, 10)` returns `r log(1000, 10)` (@fig-args **B**). 
...
In this book, I use the "native pipe," `|>`. This was added to base R in May of 2021, after [various pipes](https://magrittr.tidyverse.org/reference/pipe.html) introduced in  tidyverse's [`magrittr`](https://magrittr.tidyverse.org/) package have proved massively useful. 
