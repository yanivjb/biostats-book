## • 15. Assumptions of t {.unnumbered #t_assumptions}
...
**Motivating scenario:** Statistical modelling allows us to test null hypotheses and quantify uncertainty. But results from these models are only as good as the data and how well those data match the assumptions of the analysis. Here we learn  how to check these assumptions, and what to do when assumptions are violated. 
...
Modeling uncertainty and testing null hypotheses requires an appropriate sampling distribution. But remember our challenge - we are stuck with a single sample, so we do not have a sampling distribution.  To approximate this distribution, we must make some assumptions. For resampling methods like bootstrapping and permutation, we assume the data are independent and collected without bias. Parametric methods, like the t-test, use a well-defined statistical distribution (here, the t-distribution) as a shortcut to get a sampling distribution. This shortcut is only valid if its own assumptions are met. When using the t-distribution, we assume: 
  

- Data are independent.   
- Data are collected without bias.  
- The mean is an appropriate summary of the data.  
...
The robustness of our inference depends on which assumptions are broken, and how severe these violations are. For example, because the central limit theorem states that any sampling distribution approaches a normal distribution as sample sizes get large, the t-distribution is robust to modest deviations from normality.  

But, of course, we cannot ignore all violations of model assumptions. Here's  what to do if our data violate different assumptions:   
...
- **Are the data independent?** I'm not sure about this. We see that some locations and some taxa appear more than once (@fig-nonindep). This suggests the data may not be fully independent. For example, what if there's something unique about low-elevation regions in the UK? This isn't necessarily a reason to stop the analysis, but it's worth considering.  
...
- **Is the sample (or more specifically, the sampling distribution) normal?** My sense is that the data are normal enough for this analysis. But you can view the data and decide for yourself. As a guide, the image below displays nine quantile-quantile plots -- one shows our data while the other eight were generated with the [`rnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html) function. Can you easily tell which one is from the real data? I sure can't, so this seems normal enough to me! 
...
<div 
  ...
  <img 
    ...
    alt="The same 3x3 grid, but panel 4 is highlighted in yellow, while the other eight are highlighted in red." 
...
Although the data may not be fully independent, it roughly meets the assumptions of the t-distribution. Now that we've evaluated our assumptions let's get going!