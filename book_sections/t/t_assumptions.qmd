...
**Motivating scenario:** Statistical modelling allows us to test null hypotheses and quantify uncertainty. But results from these models are only as good as the data and how well those data match the assumptions of the analysis. Here we learn  how to check these assumptions, what to do when assumptions are violated. 
...
- **Are the data independent?** I'm not sure about this. We see that some locations and some taxa are there more than once (@fig-nonindep). My sense is that this means the data may not be fully independent. What if there's something unique about low-elevation regions in the UK? This isn't necessarily a reason to stop the analysis, but it's worth considering.  
...
- **Is the sample (or more specifically, the sampling distribution) normal?** My sense is that the data are normal enough for this analysis. But you can view the data and decide for yourself. As a guide, the image below displays nine quantile-quantile plots -- one shows our data while the other eight were generated with R's function to generate a random sample from the normal distribution, [`rnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html). Can you easily tell which one is from the real data? I sure can't, so this seems normal enough to me! 
