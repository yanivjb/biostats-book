## • 15. The paired t-test  {.unnumbered #paired_t_test}
...
**Motivating Example:** You want to look into the differences between treatments in an experiment where pairs of otherwise identical individuals receive different treatments. 
...
A common use of the one-sample t-test is to compare groups when there are natural pairs in each group. These pairs should be similar in every way except for the treatment difference we are investigating.  
...
We can therefore test if the hybridization rate differs across locations. Here we focus on two locations -- Lower Breckenridge (LB) and Sawmill Road (SR).   The data are presented in  @fig-paired1. Panels A and B show the same data in unpaired and paired form; panel C shows the distribution of within-RIL difference.

:::aside 
**The next chapter introduces the two-sample t-test.** A paired t-test generally provides more statistical power to reject the null hypothesis and estimates the difference in means with less uncertainty than a two-sample t-test. However, the two-sample t-test is useful because it can be applied when data are not paired.  
:::

Below I introduce the paired t-test, which tests the null hypothesis that the mean difference between pairs is zero. To do this, we run a one-sample t-test on the differences within each pair. This means our degrees of freedom equals the number of pairs minus one. By accounting for variation between RILs, the paired t-test focuses on differences within each pair. This often increases statistical power (i.e. our ability to reject a false null hypothesis).
...
Because a paired t-test is simply a one-sample t-test on the differences in each pair, all our assumptions apply to the distribution of differences within pairs. In other words, the paired test converts two measurements per pair into a single variable: the difference.
...
But we're not looking for a perfect normal distribution. We want the data to be normal enough so that we believe in our statistics. Because the t-test is robust to minor violations of the normality assumption, and because our qq-plot (@fig-qq_diffs) doesn't look so bad, we can move on and do some stats.  
...
While you are free to report means and confidence intervals for each "treatment" in a paired t-test, we are actually focused on the differences between pairs. In the tibble below, I have calculated this as:   
...
<div style="text-align: center; color: darkred; font-size: 3em;">
<strong>🛑 STOP 🛑</strong>
</div>



Before doing the whole NHST thing, you should immediately notice two things:   

1. Because Cohen's D is in the "tiny" (0.01 – 0.20) range, even if the estimated effect was true, it would not be particularly  strong.   
2. This will not be significant at the $\alpha = 0.05$ level -- results will never be statistically significant when the absolute value of t is less than 1.96.   
...
If your data are formatted like these (see margin), the [`t.test()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/t.test.html) function provides  two equivalent ways to conduct a paired t-test. Both approaches give the same result, since they are just two ways of formulating the same test. I show you how to do them below:   
