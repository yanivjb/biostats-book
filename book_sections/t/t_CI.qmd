## • 15. Uncertain-t {.unnumbered #t_CI}
...
**Motivating Example:** You have estimated the mean of a single continuous variable, but you need to report its uncertainty. In this section we'll use the t-distribution to find the 95% confidence interval.   
...
As you can see, the only difference between the standard error when the population standard deviation is known ($\sigma_x$) and the standard error used with the t-distribution ($s_x$) is that we replace the unknown population standard deviation ($\sigma$) with its estimate from the sample ($s$).

The difference between $\sigma$ and $s$ comes from how they are calculated:  

- The population standard deviation ($\sigma$) divides the sum of squared deviations from the true population mean ($\Sigma (x_i - \mu)^2$) by $n$ and then finds the square root.   
- The sample standard deviation ($s$) divides the sum of squared deviations from the estimated population mean ($\Sigma (x_i - \bar{x})^2$) by $n - 1$  and then finds the square root.    

This adjustment reflects the fact that once we estimate the mean from the data, we lose one degree of freedom.  
...
#| fig-alt: "A scatterplot showing the critical value of the t-distribution (α = 0.05) versus degrees of freedom from 2 to 25. The critical value starts above 4 for very small degrees of freedom and steadily decreases as degrees of freedom increase, approaching about 2. A dashed red horizontal line at 1.96 shows the corresponding critical value from the normal (Z) distribution, illustrating that t critical values approach the Z value as sample size increases."
...
3. Reporting the $(1-\alpha)\%\text{ CI }$ as $\overline{x} \pm t_{\alpha/2} \times s_\bar{x}$, where   $t_{\alpha/2}$ is the critical two-tailed t value for a specified $\alpha$ level, which we can find in R using the [`qt()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/TDist.html) function.    
    - Critical values for a 95% confidence interval with two to twenty-five degrees of freedom are shown in @fig-tcrit. Note that this approaches 1.96 (the Z-critical value) as $df \to\infty$.
...
Because we know that roughly 95% of the Z is between two and negative two, and because the t-distribution approaches the Z distribution as the degrees of freedom increase, we can guesstimate the 95% CI as  $39.33 \pm 2 \times 5.51$. i.e. The 95% CI is roughly between 28 and 50. 
...
The *q_()* family of functions are available for many common distributions. For example *qnorm(.025, lower.tail = FALSE)*, returns `r qnorm(.025, lower.tail = FALSE) |> round(digits = 4)`, the Z value that marks the upper 2.5% of the Z distribution.
:::
