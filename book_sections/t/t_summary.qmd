## • 15. t Summary {.unnumbered #t_summary}
...
Links to: [Summary](#t_summary_chapter-summary). [Chatbot tutor](#t_summary_chatbot_tutor).  [Questions](#t_summary_practice-questions). [Glossary](#t_summary_glossary-of-terms). [R functions](#t_summary_key-r-functions). [More resources](#t_summary_additional-resources).
...
#| column: margin
#| label: fig-teacherst
#| fig-cap: "[T distribution adapted from xkcd](https://xkcd.com/1347/). The rollover text says \"If data fails the teachers T-test you can force it to take it again until it passes.\" See the related [explainxkcd](https://www.explainxkcd.com/wiki/index.php/1347:_t_Distribution) for more info!"
...
Please interact with this custom chatbot ([**link here**](https://chatgpt.com/g/g-68b5014fafb881919923b5a1b582b79e-t-distribution-tutor)). I have made it to help you with this chapter. I suggest interacting with at least ten back-and-forths to ramp up  and then stopping when you feel like you got what you needed from it. 
...
**Q2.** If our sample is not perfectly normal:  `r longmcq(c( "We cannot use t-based tests, as they assume normality, and are not useful if data aren't normal", answer = "We can use t-based tests so long as the sample is sufficiently large, and the deviations from normality aren't too big, so the central limit theorem bails us out.", "This has no impact on t - that's why we use a t-distribution instead of a z distribution"))`


---

**Q3.**  The t and z distributions are never perfectly identical. But when will they be super close to one another?   `r longmcq(c( answer = "When the sample size is large","When the standard deviation is small","When data are normal", "Never, these are very different distributions - this is akin to asking when an orange and apple are similar."))`
...
**Q6.** The difference in laughter between subordinate and dominant hyenas is ___ **standard errors** away from zero. `r fitb(2.85,tol = 0.051,num = TRUE)`.
...
**Q7*.* The 95% CI is calculated as the mean ± the critical t-value at 𝛼= 0.05 times ___ . `r mcq(c("Cohen's d", "The standard deviation", "The variance", "t", answer = "The standard error", "1.96"))`
...
`qt()` also takes an optional argument, `lower.tail`. Setting this to `FALSE` returns a positive number. 
...
**Q9.** Given the answers to *Q7* and *Q8* do you think the difference between subordinates and dominants is "statistically significant" (i.e. would you reject the null) at the $\alpha = 0.05$ level? `r longmcq(c(answer = "yes because t is greater than the critical value", "no because t is greater than the critical value","we can't know this until we find our p-value"))`
...
## 📊 Glossary of Terms {#t_summary_glossary-of-terms} 
...
* **Effect Size**: A quantitative measure of the magnitude of a phenomenon; here it describes how far the sample mean is from the null hypothesis mean, in standardized units.
    * **Cohen’s d**: A standardized effect size calculated as the mean difference divided by the sample standard deviation, $d = (\bar{x} - \mu_0)/s$.
...
## 🛠️ Key R Functions{#t_summary_key-r-functions}  

 
:::functions

- [`pt():`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/TDist.html) Finds cumulative probabilities for a t-distribution (area under the curve up to a value).   

- [`qt():`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/TDist.html) Finds critical values (quantiles) for a t-distribution, e.g. for confidence intervals.   

- [`t.test():`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/t.test.html) Performs one-sample, two-sample, or paired t-tests in R.    
...
- [`lm():`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/lm.html) Fits linear models; an intercept-only model (`lm(y ~ 1)`) is equivalent to a one-sample t-test.   

- [`geom_qq()`](https://ggplot2.tidyverse.org/reference/geom_qq.html) and [`geom_qq_line()`](https://ggplot2.tidyverse.org/reference/geom_qq.html): Create QQ plots to visually check whether data are close enough to normal. 
