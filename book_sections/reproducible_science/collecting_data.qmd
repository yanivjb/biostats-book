...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/1MmhcJY7tQcsQaueTrQEO9Ok4Z--ivWxJ/view?usp=sharing) &nbsp;·&nbsp; 🎧 [Listen](../../audio/intro_to_r/reproducible_science/collecting_data.mp3)
:::
...
Similarly, it's important to have consistent and unique names. For example, my collaborators and I study *Clarkia* plants at numerous sites, including **S**quirrel **M**ountain and **S**aw**M**ill. Which one should we call SM? These questions highlight the need for thoughtful data planning, ensuring that every variable is measured consistently.

:::aside

There is a similar issue in state abbreviation -- we live in **M**i**N**nesota (MN), not **MI**nnesota, to differentiate our state from **MI**chigan (MI). See [Gary Gulman's description of the history behind this challenge.](https://www.youtube.com/watch?v=dLECCmKnrys).  
:::
...
-  *Be consistent and deliberate:* You should refer to a thing in the same thoughtful way throughout a column. Take, for example, gender as a nominal variable. As shown in the margin, we should ensure that each level (male, female, and non-binary) is entered in one way. Data validation approaches (described below), can help you enforce the convention you land on.
...
- **No values should be implied.** Never leave an entry blank as "shorthand for same as above". Similarly, never denote the first replicate (or treatment, or whatever) by its order in a spreadsheet, but rather make this explicit with a value in a column. Data order could get jumbled, and it would take real effort to go from implied order to a statistical analysis. It is ok to skip this while taking data in the field, but make sure no values are implied when entering data into a computer.       </br></br>     
...
- **Data should be tidy  (aka rectangular).** Ideally data should be entered in tidy format. This means that each variable must have its own column, each observation must have its own row, and  each value must have its own cell. See the [next chapter](#data_in_R) for more explanation). However, you must balance two practical considerations -- "*What is the best/easiest/fastest way to collect and enter data?*" vs "*What is the easiest way to analyze data?*"
