...
3. Use the [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html) function in `R` to add or overwrite columns.     
4. Use the [`filter()`](https://dplyr.tidyverse.org/reference/filter.html) function to choose the rows you want to work with.  
   - Recognize that two functions in different packages may share the same name (e.g., `filter()`), and know how to resolve these conflicts when errors occur.  

5. Combine these operations with the pipe  [`|>`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/pipeOp.html) operator.  
6. Generate a reproducible R script which documents your data wrangling efforts.
:::     

::: {style="font-size: 0.85em; color: #6b7280; margin-top: 0.8em; margin-bottom: 0em;"}

**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/11tXsVcFsNGsWhHz24JJxOfiV7iaHvVhv/view?usp=sharing) &nbsp;·&nbsp; 🎧 [Listen](../audio/intro_to_r/data_in_R.mp3)
:::

---


<article class="drop-cap">Science relies on the collection and analysis of data to understand the  world. Data are the backbone of all scientifically informed decisions. But data on its own is just a pile of numbers. We must analyze data and make sense of it. Here, I introduce tools for the standard scientific tasks of organizing, exploring, and dealing with data. Throughout we focus on how to do this reproducibly.  </article> 
...
So tidy data is not necessarily "prettier" or easier to read, or an efficient use of computer memory -- in fact, when preparing data table fora human audience, we often choose an untidy format. But when analyzing data on our computer, a tidy format simplifies our work. For this reason we will work with tidy data when possible in this book. 
...
A tibble is the name for the primary structure that holds data in the tidyverse. Tibbles have no  magical property that makes data tidy,  but tibbles do encourages a structured, consistent format that works well with tidyverse functions.
...
- [Examining the data structure and selecting and renaming columns](#select_vars).   
- [Adding columns with mutate](#add_vars).    
- [Choosing rows](#choose_rows).
- [Example R code](#dealing_w_data_script) to implement these ideas in a realistic scenario.
