...
**Motivating scenario:**  You can compare two groups with a two-sample t-test. You also read my repeated words "that a two-sample t-test is just a specific linear model." So you are now ready to build from this t-test understanding to the wider world of linear models. 
...
<article class="drop-cap">The $F$ distribution is often introduced after the t-distribution to clear a path to comparing means of more than two groups. This makes sense, as the $F$ distribution lends itself to this application. But, in my view at least, that introduction to the $F$ does it a disservice. More than simply offering a way to compare more than two groups, the $F$ provides a new way to think about variability in data. Specifically, the $F$ allows us to "partition variance" into components associated with predictors in our model and "residual" variance that is not in our model. </article><br>
...
:::aside
[Introgression](https://en.wikipedia.org/wiki/Introgression) is the incorporation of ancestry from one lineage into the genome of another. This happens through hybridization and subsequent backcrossing. 
:::
