## • 20. P-values & NHST {.unnumbered #evaluating_significance}
...
* Avoid the mistake of using [`anova()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/anova.html) to find p-values for terms in a multiple regression, and explain why these are wrong. 
* Avoid the mistake of using coefficient p-values from [`summary()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/summary.lm.html) and [`tidy()`](https://generics.r-lib.org/reference/tidy.html) as term-level tests, and explain why these can be misleading.
:::
...
This means that if we accidentally use Type I sums of squares the p-value for a term can literally depend on the order it is entered into the lm() function. Because R's [`anova()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/anova.html) function uses Type I sums of squares, we should be really careful about this. 

To see why this matters, consider the following tests for an association between anther-stigma distance (`asd_mm`) and the proportion of hybrid seeds (`prop_hybrid`) in a model accounting for numerous additional covariates:   

- With Type I sums of squares, I wrongly conclude that anther-stigma distance is associated with `prop_hybrid` if `asd_mm` is entered first. This is because the p-value is computed before accounting for the other terms.   
...
- Or correctly conclude that there is no such association if `asd_mm` is entered last:  
...
:::aside
In the [ANOVA](#post_hoc) section, we used [`multcomp`](https://cran.r-project.org/web/packages/multcomp/index.html)'s [`glht()`](https://search.r-project.org/CRAN/refmans/multcomp/html/glht.html) function for a post hoc test. We can also do this for a multiple regression (but be sure to convert location to a factor) if you use the [`glht()`](https://search.r-project.org/CRAN/refmans/multcomp/html/glht.html) function. 
