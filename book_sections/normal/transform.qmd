## • 14. Make It Normal  {.unnumbered #normal_transform}
...
**Motivating scenario:**   After looking at your data you can tell it's pretty far from normal. You know that simply ignoring this violation could lead to incorrect conclusions. What do you do? Data transformation is a valid way to change the shape of your data so that it better meets the assumptions of your statistical model.
...
We have already discussed option 1 at length, and will return to options 3 and 4 later in the term. Here we visit option 2, transformation, in which we change the shape of our data. This builds on our earlier [introduction to transformation](#changing_shape).
...
- **Choose your transformation before conducting your statistical tests.** Don't let results of your statistical analysis influence your decisions about  transformation.  
...
| ---- | --------------------- | ------------------ |
| Log  | $(Y'=\log(Y+\epsilon))$ | Right-skewed       |
...
We have seen that the normal distribution arises when we add up a bunch of things.  If we multiply a bunch of things, we get a "log-normal" distribution. Because adding logs is like multiplying untransformed data, a log transform makes exponential data look normal.   

Take our distribution of the petal area of *parviflora* RILs we've been using all chapter. You will note that we have been using the log-scaled data. We have done so because  petal area is initially far from normal (@fig-petal_area A,B), but after a log transformation it is much closer to normal (@fig-petal_area C,D) and would likely have a normal sampling distribution for relatively modest sample sizes.   
...
#| label: fig-norm_meme
#| column: margin
#| fig-alt: "A normal distribution is shown with three characters. At the low and high ends of the curve are characters representing simple and expert approaches, both labeled \"Bootstrap for uncertainty, Permute for NHST.\" In the large, central part of the curve is a crying character overwhelmed by a list of complex statistical models like \"Linear Model\" and \"GLMM.\" This is an adaptation of the popular meme."
#| fig-cap: "A meme to contrast different approaches to statistical analysis, based on perceived complexity versus wisdom. Bootstrapping and permuting (when properly reflecting the structure of our data) often solve both simple and complex stats problems."
...
Not all data can be made to be normal. If you are worried that even after transformation your data are too far from normal for standard linear models, don't despair! We can build models that better meet the data, permute/bootstrap (@fig-norm_meme), or find a better test. 