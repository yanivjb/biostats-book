## • 14. The Normal is Common  {.unnumbered #clt}
...
Use the *Sample Size (n)* slider for each variable, and evaluate how large `n` needs to be before the QQ plot points form a straight line, signaling that the sampling distribution has become approximately normal.
...
`r hide("Explanation")`
At n=25, the QQ plot is reasonably straight, showing the CLT has taken effect.
`r unhide()`

---

**Q2.**  What is the minimum sample size that allows us to reasonably approximate the **proportion hybrid** data with a normal distribution?
