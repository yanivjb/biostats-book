## • 14. Normal Introduction {.unnumbered #normal_intro}
...
2.  Explain the concept of a probability density  for continuous data and how it differs from a probability mass function for discrete data.</br></br>
3.  Use R's `dnorm()` function to find the probability density of a specific value from a normal distribution.  
     - And explain why this is not the same thing as the probability of that exact value.
4.  Know that the standard error, $\sigma_\bar{x}$, is the standard deviation of the sampling distribution.     

     - The standard error of a sample from a normal distribution is $\sigma_\bar{x} = \frac{\sigma}{\sqrt{n}}$ where:    
        - $\sigma = \sqrt{\frac{\Sigma(x_i-\mu)^2}{n}}$ is the population standard deviation,  
...
#| fig-cap: "The normal distribution that fits the log-10 transformed petal area data. The curve shows the probability density for any given value, pretending our sample means and standard deviations were actually parameters in a normal distribution. This raw probability density is proportional to the line in the first figure in this subsection, but is not scaled to match our sample size or the bin width of the histogram."
ggplot(gc_rils, aes(x = log10_petal_area_mm))+
...
- But some observations would be more surprising  than others
    - A flower with a $log_{10}$-transformed petal area of 99 would shock me.   
    - A flower with a $log_{10}$-transformed petal area of 1.873892 would be less surprising.  




    
To address this we work, not with raw probabilities, but with probability densities -- numbers whose area over an interval corresponds to probability. **The probability density function of the Gaussian (aka normal) distribution equals** 
...
This means that rather than summing to one (as traditional "**probability masses**" for categorical or discrete variables do) the probability density integrates to one. Thus, unlike probability masses, probability densities (like the one above, and many points in @fig-dnorm) can exceed one.  


### The Probability Density of a Sample Mean

Remember that the standard deviation of the sampling distribution of the mean is called the standard error. To calculate the probability density of a sample mean from samples of size $n$, we keep the same mean, $\mu$, but replace the original standard deviation, $\sigma$, with the standard error, $\sigma_{\bar{x}}=\frac{\sigma}{\sqrt{n}}$.  

For instance: the probability density that a random draw of size n from a normal distribution with mean 1.781 and sd 0.099 will have a sample mean of 1.874 equals:  

- **For $n = 2$:** `dnorm(1.874,  mean = 1.781, sd = 0.099/sqrt(2))`=`r dnorm(1.874, mean =  1.781, sd = 0.099 / sqrt(2) )|>round(digits = 3)`.   

- **For $n = 10$:** `dnorm(1.874, mean = 1.781, sd = 0.099/sqrt(10))`=`r dnorm(1.874, mean = 1.781, sd = 0.099/sqrt(10))|>round(digits = 3)`.  
...
This is a mathematical approximation, not the true description of this population for three reasons.    
...
But as long as we keep these caveats in our heads using the normal distribution to approximate these data is incredibly useful!
:::