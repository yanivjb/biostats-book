## • 14. Normal Math  {.unnumbered #p_norm}


::: {.motivation style="background-color: #ffe6f7; padding: 10px; border: 1px solid #ddd; border-radius: 5px;"}

**Motivating scenario:** You want to use R to calculate the probability that a draw from a normal falls in a certain range.
...
2.  **Apply basic probability theory** to use output from `pnorm()` to find the probability that a draw from a normal lands in some predetermined range.  
...
I introduced a webapp that integrates the area under the standard normal curve for you. Here, I introduce the [`pnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html) function, and show you how to use it to have R, rather than a webapp, calculate this area. [Later](#noRmal), I will introduce how to use [`rnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html) to simulate this process.

This is useful because we often want to calculate a p-value, which amounts to asking "*How much of the null sampling distribution is more extreme than my observed test statistic?*"

### Intro to [`pnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html)
...
#| fig-alt: "A bell-shaped normal distribution with mean 1.78 and standard deviation 0.1. The curve peaks at the mean and tapers symmetrically on both sides. The regions more than 1.5 standard deviations from the mean are shaded in blue: values less than 1.63 on the left and greater than 1.93 on the right. The left shaded region is labeled lower.tail = TRUE, and the right shaded region is labeled lower.tail = FALSE, illustrating how pnorm() calculates probabilities in the lower or upper tail."
mu           <- 1.78
sigma        <- 0.1
...
ggplot(curve_data, aes(x = x, y = y)) +
  ...
  labs(
    ...
    subtitle = "Shaded areas represent values more than 1.5 sds away from the mean.",
    x = "Value",
...
- `lower.tail`: Whether we want the upper or lower tail (see @fig-tails-c).      
...
:::aside
**Remember the notation:** $\mathcal{N}(1.78,\,0.01)$ means a normally distributed random variable with mean 1.78 and variance 0.01 (i.e. standard deviation of $\sqrt{0.01}=0.1$).
:::
...
- Reject $H_0$ if $p < \alpha$ (where $\alpha$ is traditionally 0.05). Otherwise, fail to reject $H_0$. 

:::note 
Finding the Z statistic should remind you of the Z-transform, but rather than transforming each data point, we transform our data summary. As such we subtract the proposed value under the null, $\mu_0$, from our observed value, $\bar{x}$, and divide by the standard error (i.e. the standard deviation of the sampling distribution).
