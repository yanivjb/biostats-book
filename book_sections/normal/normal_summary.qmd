...
* **Z-test**: A test of the null hypothesis that a population's true mean equals its proposed null value, $\mu_0$. To do we find the distance (in standard errors) between our estimate, $\bar{x}$, and its null value $\mu_0$, as $Z=\frac{\bar{x}-\mu_0}{\sigma_\bar{x}}$, where $\sigma_\bar{x}$ is the standard error. We then find the two-tailed p-value as `2 * pnorm(q=abs(Z), sd =1, lower.tail = FALSE)`.
