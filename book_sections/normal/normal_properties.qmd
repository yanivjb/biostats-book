## • 14. Normal Properties {.unnumbered #normal_properties}
...
4.  Explain a Z-score and how to change any normal distribution into the standard normal distribution.
...
- **The normal distribution is symmetric about its mean.** This means that if you  printed the normal distribution on a piece of paper, and folded it in half at the mean, one side would lay perfectly atop the other.  
...
- **$\approx 95\%$ of the normal distribution is within two standard deviations of the mean.** 
...
- **To see that the $\approx 95\%$ of the normal distribution is within $2\times \sigma$ of the mean:**     
...
The answer is the **Z-transformation**, a simple formula that allows us to convert any value from any normal distribution into an equivalent value on the standard normal distribution. This means we can solve any normal distribution problem by converting it to the standard normal first.

We Z-transform by taking each value, subtracting the mean, and dividing the result by the standard deviation.  So, if the $i^{th}$ observation has a value of $x_i$, its Z-value is:
