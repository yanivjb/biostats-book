## • 14. Is It Normal?  {.unnumbered #is_it_normal}


::: {.motivation style="background-color: #ffe6f7; padding: 10px; border: 1px solid #ddd; border-radius: 5px;"}

**Motivating scenario:**  Many common statistical approaches assume that your data (or the model's residuals) are normally distributed. So before we can run and interpret such models we must be able to evaluate whether this assumption is fair. Here I show you how to do that!  
...
format: html
shinylive:
  packages: ['dplyr', 'forcats','ggplot2','munsell','bslib']
...
I’m always surprised by how easily I can convince myself that a sample doesn’t come from a normal distribution. Try clicking the *Generate a sample from the normal distribution* button in the app below a few times, and experiment with the sample size to get a sense of how variable samples from a normal distribution can be.
...
Let's compare the samples from a normal distribution in our Shiny app above, to cases in which the data are not normal.   For example,  

- **@fig-msleepqq A-D** makes it clear that across the three *Iris* species, petal length is bimodal.
- **@fig-msleepqq E-H** makes it clear that across all mammals the distribution of body weights is approximately exponential.  

These examples are a bit extreme. Over the course of the book, we'll get practice in visually assessing if data are normal-ish.
