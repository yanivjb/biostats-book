## Optional: Simulating from the Normal Distribution  {.unnumbered #noRmal}
...
1.  Use R's [`rnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html) function to simulate random draws from any normal distribution.
2.  Simulate a sampling distribution of the mean of a normally distributed random variable.
3.  Use [`rnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html)'s output to verify the theoretical properties of the normal distribution.
...
Earlier, I showed you how to use a web app or R's [`pnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html) function  to calculate probabilities from a normal distribution. Here, I introduce R's [`rnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html) function to generate random data from the normal distribution. The simple [`rnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html) simulation below will give us the same answer as our calculations above. I teach simulation because I think its randomness builds intuition, and because simulation also works in more complex cases where there isn't a mathematical answer.


## Simple Simulation with [`rnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html)   
...
- We first simulate the data with [`rnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html) (see simulated data in the margin).    
...
:::note
[`rnorm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html) (and all `_norm()` functions) defaults to the standard normal, so `rnorm(n = 10)` generates a sample of ten observations from a normal distribution. 
:::
...
We can calculate the standard deviation of the sample means in @fig-norm_sampling_dist as shown in the margin. You can see that the standard deviation of the sampling distribution of the standard normal with a sample size of 9 is approximately $1/3$, which reassuringly is the standard deviation ($\sigma = 1$) divided by the square root of the sample size, $\sqrt{n} = \sqrt{9} =3$.
