### • 2. Two categorical variables {.unnumbered #two_categorical}
...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/1MrrRMKjQub6nJepAGPSIslkzDcuuCbY0/view?usp=sharing) &nbsp;·&nbsp; 🎧 [Listen](../../audio/intro_to_r/intro_to_ggplot/two_categorical.mp3)
:::

::: {.callout-note collapse="true"}
## Unscroll for code to access the `ril_data` & `ril_data_binary` datasets for this section
...
We saw that many plants received no visits, so we might prefer to compare the proportion of plants that did and did not receive a visit from a pollinator by some explanatory variable (e.g. petal color or location). 

Removing plants without a known petal color and changing visits from a number to  yes/no (aka boolean) format requires tricks we will learn in the chapter on [dealing with data in R](#data_in_R). In the hidden code below, I used these tricks, to make the object, `ril_data_binary`. Unfold the box below to see how.
...
- If data are aggregated (i.e. a summary of a huge dataset with counts for each combination of variables) use [`geom_col()`](https://ggplot2.tidyverse.org/reference/geom_bar.html)
...
We need to plot these data somewhat differently. There are two key differences: 
...
aggregated_pollinator_obs <- ril_data_binary |>
