...
Removing plants without a know petal color and changing visits from a number to  yes/no requires tricks we will learn in the chapter on [dealing with data in R](#data_in_R). In the hidden code below, I used these tricks, to make the object, `ril_data_binary`. Unroll the box below to see how
