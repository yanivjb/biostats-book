...
Removing plants without a known petal color and changing visits from a number to  yes/no (aka boolean) format requires tricks we will learn in the chapter on [dealing with data in R](#data_in_R). In the hidden code below, I used these tricks, to make the object, `ril_data_binary`. Unroll the box below to see how
...
- If dara re aggregated (i.e. a summary of a huge dataset with counts for each combination of variables) use [`geom_col()`](https://ggplot2.tidyverse.org/reference/geom_bar.html)
