### • 2. Continuous y/categorical x {.unnumbered #cont_cat}
...
**Alternative formats:** 🎥 [Watch](https://drive.google.com/file/d/1S4uLnuHrPGUeYTPpi59jL6381vcurznj/view?usp=sharing) &nbsp;·&nbsp; 🎧 [Listen](../../audio/intro_to_r/intro_to_ggplot/continuousy_categoricalx.mp3)
:::


::: {.callout-note collapse="true"}
## Unscroll for code to access the `ril_data` and `gc_sr_rils` datasets for this section
...
gc_sr_rils <- ril_data |> 
  dplyr::filter(location == "GC" | location == "SR")
```
...
- Filter our data for samples from \"GC\" and \"SR\" (because we did not conduct pollinator observations at the other sites).   We have not yet learned this so I make this reduced data set, called `gc_sr_rils` in the margin here. 
- Use the new data, `gc_sr_rils` in our `ggplot()` function.  
